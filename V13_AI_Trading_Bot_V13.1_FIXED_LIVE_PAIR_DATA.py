    ...
    def rank(symbols, analyses):
        ...
        for sym in symbols:
            ...
            rows.append(
                {
                    "SYMBOL": sym,
                    "DIRECTION": c.get("direction", "NO DATA"),
                    "SCORE": round(float(c.get("score", 0.0)), 1),
                    "REGIME": a.get("regime", "NO DATA"),
                    "VOLATILITY": a.get("volatility", {}).get("regime", "NO DATA"),
                    "SESSION": a.get("session", {}).get("session", "NO DATA"),
                    "TREND": a.get("trend", {}).get("label", "NO DATA"),
                    "MOMENTUM": a.get("momentum", {}).get("direction", "NO DATA"),
                    "STRUCTURE": a.get("structure", {}).get("direction", "NO DATA"),
                    "ENTRY QUALITY": round(float(c.get("quality", 0.0)), 1),
                    "RISK": a.get("risk", {}).get("status", "NO DATA"),
                }
            )
        return (
            pd.DataFrame(rows)
...
class LiveConnectionManager:
    ...
    def status(source, df, validation, data_quality, metadata=None):
        ...
        # A non-empty fallback/demo dataframe is NOT evidence of a live connection.
        explicit = metadata.get("connected")
        if explicit is None:
            explicit = source in ("TWELVE DATA", "MT5", "CSV") and not metadata.get("synthetic", False) and source != "FALLBACK"
        return {
            "source": source,
            "connected": bool(explicit and df is not None and not df.empty),
            "quality": data_quality.get("score", 0),
            "status": data_quality.get("status", "BAD"),
            "last_fetch": metadata.get("fetched_at", data_quality.get("last_timestamp", "-")),
            "credits_left": metadata.get("credits_left"),
            "read_only": True,
...
def init_state():
    defaults = {
        ...
        "pair_meta_cache": {},
        "pair_errors": {},
        "watchlist": ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD", "NZDUSD", "XAUUSD"],
        "symbol_sync_message": "",
    }
...
def analyze_market(df, symbol, cfg, timeframe="M5", history=None):
    ...
    # Correlation can use the live multi-pair history cache when supplied.
    if history is None:
        history = {symbol: df}
    else:
    ...
    dq = DataIntegrityEngine.assess(df, timeframe, cfg.data_max_age_seconds)
...
# ============================================================
# V13.1 LIVE PAIR DATA ORCHESTRATION
# Keeps every analysis engine synchronized with the selected pair
# and optionally supplies real histories for the market scanner.
# ============================================================
class LivePairDataEngine:
    """Unified read-only market-data router for primary and watchlist pairs."""

    @staticmethod
    def fetch_pair(symbol, timeframe, cfg):
        symbol = str(symbol).upper().strip()
        if not symbol:
            raise ValueError("Symbol cannot be empty.")

        if cfg.data_source == "TWELVE DATA":
            df, meta = TwelveDataLiveEngine.fetch(
                cfg.twelve_data_api_key,
                symbol,
                timeframe,
                cfg.twelve_data_outputsize,
            )
            meta = dict(meta or {})
            meta.update({
                "symbol": symbol,
        ...
        if cfg.data_source == "MT5":
            path = st.session_state.get("mt5_path", "") or None
            MT5LiveDataEngine.connect(symbol, path=path)
            df = MT5LiveDataEngine.bars(
                symbol,
                timeframe,
                cfg.mt5_outputsize,
            )
        ...
        # DEMO is deliberately isolated from live data.
        df = MarketDataEngine.synthetic(
            symbol,
            cfg.demo_rows,
            seed=abs(hash((symbol, timeframe))) % 100000,
        )
        meta = {
            "source": "DEMO",
            "symbol": symbol,
    ...
    @staticmethod
    def cache_pair(symbol, timeframe, df, meta):
        symbol = str(symbol).upper().strip()
        st.session_state.pair_data_cache[symbol] = df
        st.session_state.pair_meta_cache[symbol] = dict(meta or {})
        st.session_state.pair_meta_cache[symbol]["symbol"] = symbol
        st.session_state.pair_meta_cache[symbol]["timeframe"] = timeframe

    @staticmethod
    def refresh_pairs(symbols, timeframe, cfg):
        results = {}
        errors = {}
        for raw_symbol in symbols:
            ...
            if not sym:
                continue
            try:
                df, meta = LivePairDataEngine.fetch_pair(sym, timeframe, cfg)
                validation = MarketDataEngine.validate(df)
                if not validation.get("data_ok", False):
                    raise RuntimeError(
                        f"{sym}: returned data failed validation: {validation.get('reasons', [])}"
                    )
                LivePairDataEngine.cache_pair(sym, timeframe, df, meta)
                results[sym] = df
            except Exception as ex:
                errors[sym] = str(ex)
                st.session_state.pair_errors[sym] = str(ex)
        return results, errors

    @staticmethod
    def history_for(symbol, fallback_df=None):
        history = dict(st.session_state.get("pair_data_cache", {}))
        if fallback_df is not None:
            history[str(symbol).upper().strip()] = fallback_df
        return history


def _apply_pair_dataset(symbol, timeframe, cfg, df, meta):
    """Install one pair as the authoritative active dataset."""
    symbol = str(symbol).upper().strip()
    meta = dict(meta or {})
    meta.update({"symbol": symbol, "timeframe": timeframe})
    st.session_state.data = df
    st.session_state.data_meta = meta
    st.session_state.active_symbol = symbol
    st.session_state.active_timeframe = timeframe
    st.session_state.active_data_source = cfg.data_source
    st.session_state.ai_cache = {}
    st.session_state.last_signal_time = None


def synchronize_primary_pair(symbol, timeframe, cfg, force=False):
    """Synchronize symbol/timeframe/source changes without retaining stale pair data."""
    symbol = str(symbol).upper().strip()
    changed = force or any([
        st.session_state.get("active_symbol") != symbol,
        st.session_state.get("active_timeframe") != timeframe,
        st.session_state.get("active_data_source") != cfg.data_source,
    ])
    ...
    try:
        # A cached live pair is safe only when source/timeframe match.
        meta = st.session_state.get("pair_meta_cache", {}).get(symbol, {})
        cached = st.session_state.get("pair_data_cache", {}).get(symbol)
        if (not force and cached is not None and
                meta.get("timeframe") == timeframe and
                meta.get("source") == cfg.data_source):
            _apply_pair_dataset(symbol, timeframe, cfg, cached, meta)
            st.session_state.symbol_sync_message = f"✓ {symbol} · {timeframe} loaded from live-data cache"
            return True

        df, meta = LivePairDataEngine.fetch_pair(symbol, timeframe, cfg)
        validation = MarketDataEngine.validate(df)
        if not validation.get("data_ok", False):
            raise RuntimeError(f"Data validation failed for {symbol}: {validation.get('reasons', [])}")

        LivePairDataEngine.cache_pair(symbol, timeframe, df, meta)
        _apply_pair_dataset(symbol, timeframe, cfg, df, meta)
        ...
        st.session_state.symbol_sync_message = f"✓ {symbol} · {timeframe} data synchronized"
        return True
    except Exception as ex:
        # Never let old EURUSD data masquerade as the newly selected GBPUSD.
        st.session_state.pair_errors[symbol] = str(ex)
        fallback = MarketDataEngine.synthetic(
            symbol,
            cfg.demo_rows,
            seed=abs(hash((symbol, timeframe, "fallback"))) % 100000,
        )
        fallback_meta = {
            "source": "FALLBACK",
            "symbol": symbol,
        ...
        _apply_pair_dataset(symbol, timeframe, cfg, fallback, fallback_meta)
        ...
        st.session_state.symbol_sync_message = f"⚠ {symbol} live-data load failed: {ex}"
        return True
...
def dashboard():
    ...
    with st.sidebar:
        ...
        symbol = st.text_input(
            "Primary symbol",
            st.session_state.get("active_symbol", "EURUSD"),
            help="Enter the exact symbol supported by your selected data provider, e.g. EURUSD, GBPUSD, USDJPY, XAUUSD.",
        ).upper().strip()
        ...
        data_source = st.selectbox("Data source", ["DEMO", "TWELVE DATA", "MT5"], index=["DEMO", "TWELVE DATA", "MT5"].index(st.session_state.get("active_data_source", "DEMO")))
        ...
        if data_source == "TWELVE DATA":
            cfg.twelve_data_api_key = st.text_input(
                ...
                value=st.session_state.get("td_key", "")
            )
            st.session_state.td_key = cfg.twelve_data_api_key
            cfg.twelve_data_outputsize = st.number_input(
                "Historical candles", 200, 5000, 500, 100, key="td_outputsize"
            )
        elif data_source == "MT5":
            st.caption("MT5 provides broker/demo market data to V13. This adapter is read-only; it never places orders.")
            mt5_terminal_path = st.text_input(
                "MT5 terminal path (optional)",
                value=st.session_state.get("mt5_path", ""),
                help="Leave blank when the default MT5 terminal is installed.",
            )
            st.session_state.mt5_path = mt5_terminal_path
            cfg.mt5_outputsize = st.number_input(
                "MT5 historical candles", 200, 5000, 500, 100, key="mt5_outputsize"
            )

        # Primary pair always follows the selected symbol/timeframe/source.
        if synchronize_primary_pair(symbol, timeframe, cfg):
            msg = st.session_state.get("symbol_sync_message", "")
            if msg:
                if msg.startswith("✓"):
                    st.success(msg)
                elif msg.startswith("⚠"):
        ...
        if data_source in ("TWELVE DATA", "MT5"):
            if st.button("🔄 Refresh Active Pair", use_container_width=True):
                synchronize_primary_pair(symbol, timeframe, cfg, force=True)
                st.rerun()
        else:
            st.info("Demo mode: no external market-data request is made.")
    ...
    if up:
        try:
            ...
            st.session_state.active_symbol = symbol
            st.session_state.active_timeframe = timeframe
            st.success(f"Loaded {len(csv_df):,} candles for {symbol} · {timeframe}.")
        except Exception as e:
            st.error(f"CSV error: {e}")

    df = st.session_state.data

    # Hard guard: never analyze data belonging to another selected pair.
    loaded_symbol = st.session_state.get("data_meta", {}).get("symbol")
    loaded_timeframe = st.session_state.get("data_meta", {}).get("timeframe")
    if loaded_symbol and loaded_symbol != symbol:
        synchronize_primary_pair(symbol, timeframe, cfg, force=True)
        df = st.session_state.data
    elif loaded_timeframe and loaded_timeframe != timeframe and not up:
        synchronize_primary_pair(symbol, timeframe, cfg, force=True)
        df = st.session_state.data

    if cfg.data_source == "MT5" and st.session_state.get("active_data_source") == "MT5":
        tick = MT5LiveDataEngine.tick(symbol)
        if tick:
            st.info(f"🟢 MT5 LIVE · {symbol} · Bid {tick['bid']} · Ask {tick['ask']} · Spread {tick['spread']}")

    validation = MarketDataEngine.validate(df)
    data_quality = DataIntegrityEngine.assess(df, timeframe, cfg.data_max_age_seconds)
    pair_history = LivePairDataEngine.history_for(symbol, df)
    a = analyze_market(df, symbol, cfg, timeframe, history=pair_history)
    st.session_state.live_status = LiveConnectionManager.status(
    ...
    with tabs[0]:
        ...
        # Live scanner data is explicitly refreshed as a batch. No synthetic
        # substitute is used for live-data mode, so the dashboard never labels
        # generated prices as real market prices.
        st.session_state.watchlist = watch
        if cfg.data_source == "DEMO":
            for sym in watch:
                if sym not in st.session_state.pair_data_cache:
                    x_demo, m_demo = LivePairDataEngine.fetch_pair(sym, timeframe, cfg)
                    LivePairDataEngine.cache_pair(sym, timeframe, x_demo, m_demo)

        if cfg.data_source in ("TWELVE DATA", "MT5"):
            if st.button("📡 Refresh Live Watchlist", use_container_width=True):
                _, scan_errors = LivePairDataEngine.refresh_pairs(watch, timeframe, cfg)
                st.session_state.pair_errors.update(scan_errors)
                st.rerun()

        analyses = {}
        history = LivePairDataEngine.history_for(symbol, df)
        for sym in watch:
            x = history.get(sym)
            if x is None:
                analyses[sym] = {
                    "confluence": {"direction":"NO DATA","score":0.0,"quality":0.0},
                    "regime":"NO DATA",
                    "volatility":{"regime":"NO DATA"},
                    "session":{"session":"NO DATA"},
                    "trend":{"label":"NO DATA"},
                ...
                analyses[sym] = analyze_market(x, sym, cfg, timeframe, history=history)

        scanner = MarketTrackerEngine.rank(watch, analyses)
        errors = {k:v for k,v in st.session_state.get("pair_errors", {}).items() if k in watch}
        if errors:
            st.warning("Live-data issues: " + " | ".join(f"{k}: {v}" for k,v in errors.items()))

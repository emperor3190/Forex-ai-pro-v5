    ...
    def _direct_live_data(df, symbol, timeframe, cfg):
        ...
        if requested_source == "DERIV REAL FOREX":
            ...
            if frame is None or frame.empty:
                return _empty_market_data(), {"source":"DERIV REAL FOREX","direct":True,"pending":True,"stream_reused":True,"stream_health":status}
...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    try:
        if requested_source == "TWELVE DATA":
            ...
            df, meta = get_deriv_real_forex_data(
                symbol, timeframe, cfg, force=force,
                required_rows=int(getattr(cfg, "minimum_history_candles", 1000) or 1000),
            )
...
def warm_deriv_multi_pair_streams(symbols, cfg):
    ...
    def start_one(sym):
        ...
        # Warm only the reusable tick connection. Historical candles are acquired
        # on demand for the selected timeframe, never for all scanner timeframes.
...
class DerivRealForexStream:
    ...
    def _on_open(self, ws):
        with self.lock:
            ...
            # Never bootstrap every timeframe on every connection. The dashboard
            # requests only the selected timeframe; MTF consumes already-populated
            # shared stream frames and does not trigger new history calls. This
            # prevents Streamlit reruns/watchlist streams from creating request storms.
            if self.required_timeframe in self.TF_SECONDS:
    ...
    def start(self, wait_seconds=120, required_timeframe=None, required_rows=None):
        ...
        if self.thread and self.thread.is_alive():
            ...
            if self.required_timeframe:
                ...
                if snap.get("connected") and self.ws is not None:
                    ...
                    self._send_next_history_request()
            elif snap.get("connected") and self.frames:
                return snap
        ...
        if snap.get("connected") and self.frames:
            return snap
        raise RuntimeError(f"Deriv market-data stream did not produce usable candles within the startup window: {snap}")
...
def dashboard():
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            st.caption(
                "Deriv real-Forex market-data stream · read-only · "
                "uses Deriv public market data first (no PAT required). Live dashboard bootstrap is independent of the full historical-test acquisition."
            )

        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.market_data_mode == "AUTHENTICATED":
                    ...
                    ws_candidates = [legacy_url, "wss://api.derivws.com/trading/v1/options/ws/public"]
...
def get_deriv_live_market_data(symbol: str, timeframe: str, cfg: Config):
    """Read-only Deriv live path: persistent ticks -> Tick Normalizer -> OHLCV Builder.

    This function is intentionally forbidden from calling ``ticks_history``.
    Historical acquisition belongs exclusively to the explicit V14 historical
    dataset/testing workflow below. Streamlit reruns, MTF analysis, and the live
    dashboard therefore cannot spend Deriv historical-request quota.
    """
    if websocket is None:
    ...
    stream = DerivRealForexStream.get(
        ...
        cfg.deriv_stream_outputsize, cfg.deriv_stream_reconnect_seconds, market_data_mode
    )
    status = stream.snapshot()
    if not status.get("connected"):
        status = stream.start(
            wait_seconds=max(15, min(30, int(getattr(cfg, "deriv_live_start_timeout_seconds", 20) or 20))),
            required_timeframe=None, required_rows=None
        )

    # Prefer the stream's already-populated frame when historical acquisition has
    # previously been performed. Otherwise construct candles directly from the
    # observed live ticks using the V14 OHLCV Builder. No synthetic prices.
    ...
    if frame is None or frame.empty:
        # Keep the provider connection state truthful. A live stream can be healthy
        # while the OHLCV builder is still waiting for its first usable candle.
        # Do not fabricate a candle and do not mark the socket disconnected.
        meta = {
            ...
            "connection_mode": market_data_mode, "live_only": True,
            "historical_request_sent": False, "ticks_observed": int(len(ticks)),
            "stream_health": status, "live_tick_healthy": bool(status.get("live_tick_healthy")),
            "ohlcv_builder_status": "WAITING_FOR_USABLE_CANDLE",
            "fetched_at": datetime.now(timezone.utc).isoformat(),
        }
    ...
    if not validation.get("data_ok"):
        raise RuntimeError(f"Deriv {symbol}/{timeframe} live OHLCV validation failed: {validation}")
    ...
    meta = {
        ...
        "connection_mode": market_data_mode, "live_only": True,
        "historical_request_sent": False, "ticks_observed": int(len(ticks)),
        "stream_health": status, "fetched_at": datetime.now(timezone.utc).isoformat(),
    }
    return frame, meta
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            st.caption(
                ...
                "Historical candles are acquired separately from the same Deriv Forex provider through the explicit historical-data/test control. "

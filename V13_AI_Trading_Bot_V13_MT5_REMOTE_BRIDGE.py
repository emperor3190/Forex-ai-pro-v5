...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    outputsize = int(getattr(cfg, "mt5_outputsize", 500)) if requested_source in {"MT5", "MT5 REMOTE"} else int(getattr(cfg, "twelve_data_outputsize", 500))
    ...
    if not force and key in cache:
        ...
        ttl = max(1, int(getattr(cfg, "live_refresh_seconds", 15))) if requested_source in {"MT5", "MT5 REMOTE"} else float("inf")
...
def get_selected_market_data(symbol: str, timeframe: str, cfg: Config):
    ...
    if data_matches_selection(symbol, timeframe, st.session_state.get("data"), requested_source):
        if requested_source in {"MT5", "MT5 REMOTE"}:
            meta = st.session_state.get("data_meta", {}) or {}
...
def dashboard():
    ...
    with st.sidebar:
        ...
        source_options = ["MT5 REMOTE", "MT5", "TWELVE DATA", "DEMO"]

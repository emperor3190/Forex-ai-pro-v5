...
"""
V12.1 AI Trading Platform — COMPLETE BUILD
...
class TwelveDataError(RuntimeError):
    """Raised when Twelve Data cannot return usable market data."""


class TwelveDataAdapter:
    """Read OHLCV candles from Twelve Data and normalize them for V12.1."""
    ...
    def resolve_api_key(explicit_key: str = "") -> str:
        ...
        if key:
            return key
        env_key = os.getenv("TWELVE_DATA_API_KEY", "").strip()
        if env_key:
            return env_key
        try:
            secret_key = str(st.secrets.get("TWELVE_DATA_API_KEY", "")).strip()
            if secret_key:
                return secret_key
        except Exception:
            pass
    ...
    def to_twelve_symbol(symbol: str) -> str:
        ...
        if "/" in raw:
            return raw
        # Common six-letter forex notation used by this V12.1 dashboard.
    ...
    def fetch_ohlcv(cls, api_key: str, symbol: str, timeframe: str, outputsize: int = 500) -> pd.DataFrame:
        key = cls.resolve_api_key(api_key)
        if not key:
            raise TwelveDataError(
                "Twelve Data API key is missing. Add it in the sidebar, "
                "TWELVE_DATA_API_KEY environment variable, or Streamlit secrets."
            )

        interval = cls.INTERVALS.get(str(timeframe).upper())
        if not interval:
            raise TwelveDataError(f"Unsupported V12.1 timeframe: {timeframe}")

        params = {
            "symbol": cls.to_twelve_symbol(symbol),
            "interval": interval,
            "outputsize": max(100, min(int(outputsize), 5000)),
            "timezone": "UTC",
            "apikey": key,
        }
        ...
        request = urllib.request.Request(
            url,
            headers={"User-Agent": "V12.1-AI-Trading-Platform/12.1"},
            method="GET",
        )
        try:
        ...
        if str(payload.get("status", "")).lower() == "error" or "code" in payload:
            raise TwelveDataError(
        ...
        if not values:
            raise TwelveDataError(
                f"Twelve Data returned no candles for {symbol} ({timeframe})."
            )
        ...
        if missing:
            raise TwelveDataError(
                f"Twelve Data response is missing columns: {', '.join(missing)}"
            )
...
def dashboard():
    st.set_page_config(
        page_title="V12.1 AI Trading Platform",
        page_icon="📈",
        layout="wide",
    )
    init_state()
    ...
    with st.sidebar:
        ...
        st.divider()
        st.write("**📡 Market Data Connection**")
        data_source = st.selectbox(
            "Data source",
            ["Twelve Data", "CSV / Uploaded", "Synthetic / Demo"],
            index=0,
            help="Twelve Data supplies live/historical OHLCV candles to the existing V12.1 engines. CSV and Synthetic modes remain available for testing/backtesting.",
        )
        ...
        twelve_key = st.text_input(
            ...
            value="",
            type="password",
            help="For security, the key is not stored in this Python file. You can also use TWELVE_DATA_API_KEY in the environment or Streamlit secrets.",
        )
        live_outputsize = st.number_input(
            "Live candles", 100, 5000, 500, 100,
            help="Number of candles used by the existing V12.1 analysis engines.",
        )
        refresh_live = st.button("🔄 Refresh Live Data", use_container_width=True)
        if data_source == "Twelve Data":
            st.caption("Twelve Data mode: live market-data adapter enabled.")
    ...
    # Preserve the original CSV and synthetic modes while adding Twelve Data.
    if data_source == "Twelve Data":
        try:
            if refresh_live or st.session_state.get("data_source") != "Twelve Data" or not st.session_state.get("live_data_ok", False):
                live_df = TwelveDataAdapter.fetch_ohlcv(
                    twelve_key, symbol, timeframe, int(live_outputsize)
                )
                st.session_state.data = live_df
                st.session_state.live_data_ok = True
                st.session_state.live_data_error = ""
                st.session_state.live_data_last_update = datetime.now(timezone.utc)
            else:
                # Cached Twelve Data data remains available between Streamlit reruns.
                live_df = st.session_state.data
            st.success(
                f"🟢 Twelve Data LIVE: {len(live_df):,} candles for "
                f"{TwelveDataAdapter.to_twelve_symbol(symbol)} · {timeframe}"
            )
            if st.session_state.live_data_last_update:
                st.caption(
                    "Last successful fetch: "
                    f"{st.session_state.live_data_last_update.strftime('%Y-%m-%d %H:%M:%S UTC')} · "
                    "Press Refresh Live Data to request the latest candles."
                )
        except TwelveDataError as e:
            st.session_state.live_data_ok = False
            st.session_state.live_data_error = str(e)
            st.error(f"🔴 Twelve Data connection: {e}")
            st.warning("V12.1 has not replaced your existing data with invalid API data. Choose CSV/Synthetic mode or correct the API key.")
    elif data_source == "CSV / Uploaded":
        if up:
            try:
                st.session_state.data = MarketDataEngine.normalize(pd.read_csv(up))
                st.session_state.live_data_ok = False
                st.success(f"Loaded {len(st.session_state.data):,} candles from CSV.")
            except Exception as e:
                st.error(f"CSV error: {e}")
        else:
            st.info(
                f"CSV mode is active. Primary timeframe selected: {timeframe}. "
                "Upload an OHLCV CSV to replace the current dataset."
            )
    else:
        st.session_state.data = MarketDataEngine.synthetic(
            symbol, cfg.demo_rows, seed=7
        )
        st.session_state.live_data_ok = False
        st.info(
            f"Synthetic/demo data is active. Primary timeframe selected: {timeframe}."
        )
    ...
    source_label = (
        "TWELVE DATA LIVE" if data_source == "Twelve Data" and st.session_state.live_data_ok
        else data_source.upper()
    )
    st.caption(
        f"Data: {'OK' if validation['data_ok'] else 'INSUFFICIENT'} · {source_label} · UTC · "
        f"{validation['rows']:,} candles · Emergency: "
        f"{'STOPPED' if st.session_state.emergency else 'NORMAL'}"
    )
    ...
    with tabs[0]:
        ...
        for i, sym in enumerate(watch):
            try:
                if data_source == "Twelve Data":
                    x = TwelveDataAdapter.fetch_ohlcv(
                        twelve_key, sym, timeframe, min(int(live_outputsize), 500)
                    )
                    scanner_data_status.append([sym, "LIVE", len(x)])
                elif sym == symbol:
                    x = df
                    scanner_data_status.append([sym, "CURRENT DATA", len(x)])
                else:
                    x = MarketDataEngine.synthetic(sym, len(df), seed=20 + i)
                    scanner_data_status.append([sym, "SYNTHETIC", len(x)])
                analyses[sym] = analyze_market(x, sym, cfg)
            except Exception as exc:
                scanner_data_status.append([sym, "ERROR", 0])
                st.warning(f"Scanner data unavailable for {sym}: {exc}")

        if scanner_data_status:
            st.caption("Scanner data source: " + ("Twelve Data LIVE" if data_source == "Twelve Data" else data_source))
            st.dataframe(
                pd.DataFrame(scanner_data_status, columns=["Symbol", "Source", "Candles"]),
    ...
    with tabs[2]:
        ...
        st.code(
            "Market Data → Analysis → Regime → MTF → Confluence → Signal → "
            "Risk Veto → Entry → Execution → Monitoring → Journal"
        )
    ...
    with tabs[5]:
        ...
        status = pd.DataFrame(
            [
                ["Market Data", "TWELVE DATA LIVE" if data_source == "Twelve Data" and st.session_state.live_data_ok else "READY"],
    ...
    st.divider()
    st.caption(
        "V12.1 is a research/paper-trading system. No strategy is guaranteed "

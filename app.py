        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.market_data_mode == "AUTHENTICATED":
                    ...
                    # Current Deriv public market-data endpoint: no auth required.
                    ws_candidates = ["wss://api.derivws.com/trading/v1/options/ws/public"]
                    # Compatibility fallback for environments where the current endpoint
                    # is blocked or unavailable. The legacy public endpoint accepts
                    # the same no-auth market-data requests.
                    legacy_url = "wss://ws.binaryws.com/websockets/v3"
                    if self.app_id:
                        legacy_url += f"?app_id={self.app_id}"
                    ws_candidates.append(legacy_url)

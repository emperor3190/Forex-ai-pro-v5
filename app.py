...
def get_deriv_live_market_data(symbol: str, timeframe: str, cfg: Config):
    ...
    if frame is None or frame.empty:
        raise RuntimeError(
            f"Deriv live tick stream is connected but has not accumulated a usable {timeframe} candle yet. "
            f"Historical acquisition is separate and must be run explicitly for backtesting."
        )
...
def get_deriv_real_forex_data(symbol: str, timeframe: str, cfg: Config, force=False, required_rows: Optional[int] = None):
    ...
    configured_target = max(1000, int(getattr(cfg, "historical_target_rows", cfg.deriv_stream_outputsize) or cfg.deriv_stream_outputsize))
    ...
    target_rows = max(1000, int(required_rows if required_rows is not None else getattr(cfg, "minimum_history_candles", 1000) or 1000))
    ...
    stream.history_page_size = min(5000, max(1000, target_rows))
...
def _v14_render_architecture_testing(df, cfg, symbol, timeframe):
    ...
    st.caption("If the selected real provider is below the statistical minimum, V14 acquires additional provider history through the same provider path. Deriv history is paginated without relying on optional echo_req; Twelve Data uses bounded backward pagination. No synthetic candles are created.")
    st.caption(
...
def dashboard():
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            st.caption(
                "Deriv real-Forex LIVE stream · read-only · "
                "ticks → OHLCV Builder. This control never requests ticks_history; historical test acquisition is separate."
            )

...
class DerivHistoryRateGuard:
    ...
    def reserve(cls, key, min_interval=5):
        ...
        with cls._lock:
            ...
            return True, 0, "OK"
    @classmethod
    def rate_limited(cls, key, cooldown_seconds=120):
        with cls._lock:
...
class DerivRealForexStream:
    ...
    def __init__(self, symbol, api_base, app_id="", token="", account_id="", outputsize=500, reconnect_seconds=5, market_data_mode="PUBLIC"):
        ...
        self.history_request_key = f"{self.symbol}|{self.deriv_symbol}"
    ...
    def _on_open(self, ws):
        with self.lock:
            ...
            ordered = list(self.TF_SECONDS)
            if self.required_timeframe in ordered:
                ordered.remove(self.required_timeframe)
                ordered.insert(0, self.required_timeframe)
            self.history_queue = ordered
            self.history_inflight = None
            self.history_retry_count = 0
        ...
        if typ == "candles":
            ...
            try:
                ...
                with self.lock:
                    ...
                    got_page = len(built)
                    target_reached = have >= self.outputsize
...
def get_deriv_real_forex_data(symbol: str, timeframe: str, cfg: Config, force=False, required_rows: Optional[int] = None):
    ...
    stream.history_page_size = min(5000, max(1000, stream.outputsize))
    stream.history_min_interval_seconds = max(2, int(getattr(cfg, "deriv_history_min_interval_seconds", 5) or 5))

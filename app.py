...
def _td_request_allowed(cfg: Config) -> tuple[bool, int]:
    """Local per-session quota guard; avoids hammering a free 8-credit/minute key."""
    now = datetime.now(timezone.utc).timestamp()
    log = st.session_state.setdefault("td_request_log", [])
    log[:] = [float(t) for t in log if now - float(t) < int(getattr(cfg, "twelve_data_rate_guard_seconds", 60))]
    budget = int(getattr(cfg, "twelve_data_request_budget_per_minute", 7))
    if len(log) >= budget:
        oldest = min(log) if log else now
...
def _td_record_request():
    st.session_state.setdefault("td_request_log", []).append(datetime.now(timezone.utc).timestamp())


def _td_cached_pair(symbol: str, timeframe: str, cfg: Config):
    """Return a fresh cached dataset without spending an API credit."""
...
def _fetch_twelve_data(symbol: str, timeframe: str, cfg: Config):
    ...
    allowed, wait = _td_request_allowed(cfg)
    if not allowed:
        raise TwelveDataRateLimitError(
            f"V13 local API guard is protecting the Twelve Data quota. Wait about {wait}s before another live request.",
            retry_after=wait,
        )

    # Count exactly one provider request. Never issue a slash/no-slash duplicate.
    _td_record_request()
...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    if not force and requested_source == "TWELVE DATA":
...
def dashboard():
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            recent_requests = len([t for t in st.session_state.get("td_request_log", []) if datetime.now(timezone.utc).timestamp() - float(t) < 60])
            st.caption(f"V13 Twelve Data request guard: {recent_requests}/{cfg.twelve_data_request_budget_per_minute} requests in the current 60-second window. Cached candles are reused until their timeframe TTL expires.")

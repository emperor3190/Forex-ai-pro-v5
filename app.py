...
"""
V14 AI Forex Trading Bot — Frozen Architecture
...
This V14 build implements the agreed frozen architecture with strict pair/timeframe
...
class CentralAssessmentDataQualityEngine:
    ...
    def assess(
        ...
        # Central Assessment is the single hand-off point.  Preserve every
        # specialist package so downstream layers consume the exact evidence
        # that arrived here rather than bypassing the central route.
        accepted_evidence = {
            name: pkg for name, pkg in evidence_packages.items()
            if isinstance(pkg, dict)
        }
        routing = {
            name: {
                "source": "SPECIALIST_ANALYSIS_ENGINE",
                "destination": "CENTRAL_ASSESSMENT_DATA_QUALITY_ENGINE",
                "downstream": "SIGNAL_INTELLIGENCE_LAYER",
                "symbol": pkg.get("symbol"),
                "timeframe": pkg.get("timeframe"),
                "accepted": name not in invalid,
            }
            for name, pkg in accepted_evidence.items()
        }
        ...
        return {
            ...
            "accepted_evidence_packages": accepted_evidence,
            "routing_map": routing,
            "routing_status": "COMPLETE" if len(routing) == len(evidence_packages) else "INCOMPLETE",
        }

class SignalIntelligenceLayer:
    """
    Second-stage intelligence hand-off.

    Central Assessment supplies validated technical-analysis evidence.
    Market Regime, Direct Probability, AI Probability, MTF and Session are
    independent intelligence/context inputs reported directly here. COT,
    Economic Events and News/Event Filter are also reported directly here.
    None of these engines is allowed to publish a final BUY/SELL/NO-TRADE decision.
    """
    @staticmethod
    def assess(
        central: Dict[str, Any],
        evidence_packages: Optional[Dict[str, Dict[str, Any]]] = None,
        context_packages: Optional[Dict[str, Dict[str, Any]]] = None,
    ) -> Dict[str, Any]:
        calc_ts = utc_now_iso()
        evidence_packages = evidence_packages or central.get("accepted_evidence_packages", {})
        context_packages = context_packages or {}
        if not central.get("assessment_readiness", False):
            return {
                ...
                "context_evidence": context_packages,
            }

        # Directional evidence comes ONLY from Central Assessment's accepted
        # technical/market evidence. Context engines do not get counted as
        # technical votes; they are interpreted separately below.
        directions = []
        scores = []
        ...
        technical_score = float(np.clip(np.mean(scores) if scores else 0.0, 0, 100))

        # All non-technical/context intelligence is received directly here.
        # These inputs are NOT counted as Central Assessment technical votes.
        regime = (context_packages.get("MarketRegimeEngine", {}) or {}).get("evidence", {})
        direct_probability = (context_packages.get("DirectProbabilityEngine", {}) or {}).get("evidence", {})
        ai_probability = (context_packages.get("AIProbabilityEngine", {}) or {}).get("evidence", {})
        mtf = (context_packages.get("MTFConfirmationEngine", {}) or {}).get("evidence", {})
        session = (context_packages.get("SessionEngine", {}) or {}).get("evidence", {})
        cot = (context_packages.get("COTEngine", {}) or {}).get("evidence", {})
        economic = (context_packages.get("EconomicEngine", {}) or {}).get("evidence", {})
        news = (context_packages.get("NewsEventFilterEngine", {}) or {}).get("evidence", {})

        context_blocked = bool(
            str(news.get("filter_status", news.get("status", "PASS"))).upper() in
            {"BLOCK", "BLOCKED", "HIGH_RISK_BLOCK"}
            or bool(economic.get("blocked", False))
            or bool(session.get("session_tradeable") is False)
        )

        # Context alignment is descriptive and advisory. It does not create a
        # final trading signal and does not become a technical Central vote.
        regime_context = str(regime.get("context", regime.get("regime", "UNKNOWN"))).upper()
        direct_direction = str(direct_probability.get("direction", "NEUTRAL")).upper()
        ai_direction = str(ai_probability.get("direction", "NEUTRAL")).upper()
        mtf_direction = str(mtf.get("direction", "NEUTRAL")).upper()
        cot_bias = str(cot.get("bias", "NEUTRAL")).upper()

        context_directions = [d for d in (direct_direction, ai_direction, mtf_direction, cot_bias)
                              if d in {"BULLISH", "BEARISH"}]
        context_agreement = (
            100.0 * max(context_directions.count("BULLISH"), context_directions.count("BEARISH"))
            / max(len(context_directions), 1)
        )
        ...
        direct_up = _num(direct_probability, "up_probability", "probability_up", "bullish_probability")
        ai_up = _num(ai_probability, "up_probability", "probability_up", "bullish_probability")
        probability_values = [x for x in (direct_up, ai_up) if x is not None]
        integrated_probability = float(np.mean(probability_values)) if probability_values else float(technical_score / 100.0)

        technical_context_alignment = (
            "ALIGNED" if not context_directions or direction == "NEUTRAL" or direction in context_directions
            else "CONFLICT"
        )
        ...
        return {
            ...
            "technical_evidence_summary": {"bullish": bull, "bearish": bear, "neutral": len(directions)-bull-bear},
            ...
            "market_regime_evidence": regime,
            "confidence_score": technical_score,
            "context_agreement": context_agreement,
            "context_alignment": technical_context_alignment,
            "overall_intelligence_score": float((technical_score + agreement + context_agreement) / 3.0),
            "decision_readiness": bool(
                agreement >= 50 and technical_score >= 50 and context_agreement >= 50 and not context_blocked
            ),
            "context_evidence": {
                "MarketRegimeEngine": regime,
                "DirectProbabilityEngine": direct_probability,
                "AIProbabilityEngine": ai_probability,
                "MTFConfirmationEngine": mtf,
                "SessionEngine": session,
                "COTEngine": cot,
                "EconomicEngine": economic,
                "NewsEventFilterEngine": news,
            },
            "cot_bias": cot_bias,
            "economic_event_context": economic,
            "news_filter_context": news,
...
class FinalDecisionEngine:
    ...
    def resolve(
        ...
        elif not bool(gate_context.get("trade_quality_valid", True)):
            decision = "NO-TRADE"
            reason = "TRADE_QUALITY_GATE_FAILED"
        elif not bool(gate_context.get("entry_zone_valid", True)):
            decision = "NO-TRADE"
            reason = "ENTRY_ZONE_ASSESSMENT_FAILED"
        elif not bool(gate_context.get("news_filter_valid", True)):
            decision = "NO-TRADE"
            reason = "NEWS_EVENT_FILTER_FAILED"
        elif not bool(gate_context.get("engine_health_valid", True)):
            decision = "NO-TRADE"
            reason = "ENGINE_HEALTH_FAILED"
        elif not bool(gate_context.get("time_sync_valid", True)):
            decision = "NO-TRADE"
            reason = "TIME_SYNCHRONIZATION_FAILED"
        elif not bool(gate_context.get("central_routing_complete", True)):
            decision = "NO-TRADE"
            reason = "CENTRAL_ROUTING_INCOMPLETE"
        else:
...
def package_engine_evidence(
    ...
    if data_ts:
        try:
            ...
            pass
    # Evidence must remain a structured, non-decision payload.  Some specialist
    # engines historically return a string/context value; normalize that value
    # without turning it into a trading signal.
    if isinstance(evidence, dict):
        normalized_evidence = dict(evidence)
    elif evidence is None:
        normalized_evidence = {"value": None, "status": "NO DATA"}
    else:
        normalized_evidence = {"value": evidence, "status": "OK"}
...
class OHLCVBuilderEngine:
    """Builds candles from normalized ticks or returns already-built OHLCV unchanged."""
    @staticmethod
...
class ConfluenceEngine:
    ...
    def score(
        ...
        mtf = si.get("mtf_evidence", {}) or {}
        session = si.get("session_evidence", {}) or {}
        regime = str((si.get("market_regime_evidence", {}) or {}).get("context", "UNKNOWN")).upper()
        parts = {
            ...
            "Session": 3 if session.get("session_tradeable", False) else 0,
            "Liquidity": 2 if (liquidity["sweep_up"] or liquidity["sweep_down"] or liquidity["FVG"]) else 1,
            "MTF": 10 * (float(mtf.get("alignment", 0) or 0) / 100),
        }
        ...
        quality = float(
            np.clip(
                ...
                + (5 if float(mtf.get("alignment", 0) or 0) >= 80 else 0),
                0,
                100,
            )
        )
...
class V14ReplayDecisionEngine:
    ...
    def evaluate(window: pd.DataFrame, symbol: str, timeframe: str,
        ...
        # Historical replay has no live COT/event/news feed. These are still
        # represented as contextual intelligence and routed to Signal Intelligence,
        # never into the technical Central Assessment vote.
        ...
        news_filter = {"filter_status": "PASS", "blocked": False, "status": "HISTORICAL_NEWS_DATA_NOT_SUPPLIED"}
        ...
        context_packages = {}
        for engine_name, evidence in {
            "MarketRegimeEngine": {"context": re, "direction": "NEUTRAL", "status": "HISTORICAL_REPLAY"},
            "DirectProbabilityEngine": direct_probability,
            "AIProbabilityEngine": ai,
            "MTFConfirmationEngine": mtf,
            "SessionEngine": se,
            "COTEngine": cot,
            "EconomicEngine": eco,
            ...
            context_packages[engine_name] = package_engine_evidence(
                engine_id=engine_name.upper(), engine_name=engine_name, symbol=symbol,
                timeframe=timeframe, evidence=evidence, data_timestamp=data_ts,
                quality={"score": dq.get("score", 0.0), "status": "PASS" if dq.get("signal_allowed") else "BLOCK", "context_only": True},
                provenance={"source": "HISTORICAL_REPLAY", "routing": "DIRECT_TO_SIGNAL_INTELLIGENCE"},
            )
        intelligence = SignalIntelligenceLayer.assess(central, evidence_packages, context_packages)
        direction = str(intelligence.get("market_context", "NEUTRAL")).upper()

        def hev(name, default=None):
            pkg = central.get("accepted_evidence_packages", {}).get(name, {}) or {}
            return pkg.get("evidence", default) if isinstance(pkg, dict) else default

        t_c = hev("TrendEngine", {})
        m_c = hev("MomentumEngine", {})
        v_c = hev("VolatilityEngine", {})
        s_c = hev("StructureEngine", {})
        pa_c = hev("PriceActionEngine", {})
        sr_c = hev("SupportResistanceEngine", {})
        bo_c = hev("BreakoutEngine", {})
        li_c = hev("LiquidityEngine", {})
        re_c = (intelligence.get("market_regime_evidence", {}) or {}).get("context", re)
        se_c = intelligence.get("session_evidence", {}) or {}
        mtf_c = intelligence.get("mtf_evidence", {}) or {}
        c = ConfluenceEngine.score(t_c, m_c, v_c, s_c, pa_c, sr_c, bo_c, li_c, intelligence)
        market_tracker = {"direction": c.get("direction", "WAIT"), "score": c.get("score", 0), "status": "HISTORICAL_REPLAY"}
        advisory = {
            "trend": t_c, "momentum": m_c, "volatility": v_c, "structure": s_c,
            "price_action": pa_c, "sr": sr_c, "breakout": bo_c, "liquidity": li_c,
            "ema": hev("EMAEngine", {}), "macd": hev("MACDEngine", {}), "fvg": hev("FVGEngine", {}),
            "microstructure": hev("MarketMicrostructureEngine", {}), "order_flow": hev("OrderFlowVolumeFlowEngine", {}),
            "reversal": hev("ReversalDetectionEngine", {}), "regime": re_c, "session": se_c,
            "currency_strength": hev("CurrencyStrengthEngine", {}), "mtf": mtf_c,
            "economic": eco, "cot": cot, "news_filter": news_filter, "correlation": hev("CorrelationEngine", {}),
            "confluence": c, "data_quality": dq, "advanced_momentum": hev("AdvancedMomentumEngine", {}),
            "ai": intelligence.get("ai_probability", {}) or {}, "volume_flow": hev("VolumeFlowEngine", {}),
            "candle_timing": hev("CandleTimingEngine", {}), "market_region": hev("MarketRegionEngine", {}),
            "direct_probability": intelligence.get("direct_probability", {}) or {}, "market_tracker": market_tracker,
            "signal_timing": {"fresh": True, "age_seconds": 0.0, "status": "FRESH_HISTORICAL"},
        }
        ensemble = EnsembleDecisionEngine.decide(advisory, advisory["ai"], dq, cfg)
        ...
        risk = RiskControlEngine.evaluate(
            {
                ...
                "open_positions": 0,
            }, cfg, c, v_c, eco, hev("CorrelationEngine", {}),
        )
        advisory["risk"] = risk
...
def analyze_market(df, symbol, cfg, timeframe=None, *, deep_mtf=True):
    """V14 canonical evidence -> central assessment -> downstream decision path.

    Wiring contract:
      1. Market data is validated/normalized.
      2. Every specialist analysis engine publishes evidence to Central Assessment.
      3. Central Assessment is the only evidence hand-off to Signal Intelligence.
      4. Summary/decision-support layers consume centrally accepted evidence.
      5. Entry, timing, quality, news, health, synchronization and risk gates are
         passed explicitly to FinalDecisionEngine.
      6. Only FinalDecisionEngine may emit BUY/SELL/NO-TRADE.
    """
    validation = DataValidationEngine.validate(df)
    tf = timeframe or st.session_state.get("data_timeframe", "M5") or "M5"

    # ------------------------------------------------------------------
    # 1) SPECIALIST ANALYSIS — evidence only, never final signals.
    # ------------------------------------------------------------------
    ...
    mtf = (MultiTimeframeEngine.analyze(df, symbol=symbol, cfg=cfg)
           if deep_mtf else MultiTimeframeEngine.analyze(df))
    volume_flow = VolumeFlowEngine.analyze(df)
    candle_timing = CandleTimingEngine.analyze(df, tf)
    ...
    corr = CorrelationEngine.analyze({symbol: df}, symbol)
    adv_m = MomentumDirectionEngine.analyze(df)
    ai = AIProbabilityEngine.predict(df)
    dq = DataIntegrityEngine.assess(df, tf, cfg.data_max_age_seconds)

    # Context-only derived regime is explicitly labelled as context, not a signal.
    regime_evidence = {"context": re, "direction": "NEUTRAL", "status": "OK"}

    # Every specialist/analysis source is routed into Central Assessment.
    # Nothing below this list may bypass the central hand-off.
    evidence_inputs = {
        "TrendEngine": t,
        "MomentumEngine": m,
    ...
    for engine_name, evidence in evidence_inputs.items():
        if isinstance(evidence, dict):
            status = str(evidence.get("status", "OK")).upper()
        else:
            status = "OK" if evidence is not None else "NO DATA"
        # Provider/data quality is the common input-quality floor. Individual
        # engine status is preserved in the evidence itself for diagnostics.
        engine_ok = status not in {"FAILED", "ERROR", "INVALID"}
        evidence_packages[engine_name] = package_engine_evidence(
            ...
            quality={
                ...
                "status": "PASS" if dq.get("signal_allowed", False) and engine_ok else "BLOCK",
                "engine_status": status,
                "engine_output_available": bool(evidence is not None),
            },
    ...
    # ------------------------------------------------------------------
    # 2) CENTRAL ASSESSMENT -> SIGNAL INTELLIGENCE.
    # ------------------------------------------------------------------
    central_evidence = central_assessment.get("accepted_evidence_packages", {})

    # Contextual intelligence is intentionally routed directly to Signal
    # Intelligence, not through Central Assessment: COT, Economic Events and
    # News/Event Filter are context/risk intelligence, not technical votes.
    context_inputs = {
        "MarketRegimeEngine": regime_evidence,
        "DirectProbabilityEngine": direct_probability,
        "AIProbabilityEngine": ai,
        "MTFConfirmationEngine": mtf,
        "SessionEngine": se,
    ...
    context_packages = {}
    for engine_name, evidence in context_inputs.items():
        context_packages[engine_name] = package_engine_evidence(
            engine_id=engine_name.upper(),
            engine_name=engine_name,
            symbol=symbol,
            ...
            provenance={"source": dq.get("source", st.session_state.get("data_source_loaded", "UNKNOWN")),
                        "routing": "DIRECT_TO_SIGNAL_INTELLIGENCE"},
        )

    signal_intelligence = SignalIntelligenceLayer.assess(
        central_assessment, central_evidence, context_packages
    )

    def ev(name, default=None):
        pkg = central_evidence.get(name, {}) or {}
        return pkg.get("evidence", default) if isinstance(pkg, dict) else default

    # Central Assessment is now the source of truth for downstream summary layers.
    t_c = ev("TrendEngine", {})
    m_c = ev("MomentumEngine", {})
    ema_c = ev("EMAEngine", {})
    macd_c = ev("MACDEngine", {})
    v_c = ev("VolatilityEngine", {})
    s_c = ev("StructureEngine", {})
    pa_c = ev("PriceActionEngine", {})
    sr_c = ev("SupportResistanceEngine", {})
    bo_c = ev("BreakoutEngine", {})
    li_c = ev("LiquidityEngine", {})
    re_c = (signal_intelligence.get("market_regime_evidence", {}) or {}).get("context", re)
    se_c = signal_intelligence.get("session_evidence", {}) or {}
    mtf_c = signal_intelligence.get("mtf_evidence", {}) or {}
    eco_c = (context_packages.get("EconomicEngine", {}) or {}).get("evidence", {})
    cot_c = (context_packages.get("COTEngine", {}) or {}).get("evidence", {})
    corr_c = ev("CorrelationEngine", {})
    adv_m_c = ev("AdvancedMomentumEngine", {})
    ai_c = ev("AIProbabilityEngine", {})
    volume_flow_c = ev("VolumeFlowEngine", {})
    candle_timing_c = ev("CandleTimingEngine", {})
    market_region_c = ev("MarketRegionEngine", {})
    direct_probability_c = ev("DirectProbabilityEngine", {})
    news_filter_c = (context_packages.get("NewsEventFilterEngine", {}) or {}).get("evidence", {})

    # Confluence is a downstream summary of centrally accepted evidence.
    c = ConfluenceEngine.score(
        t_c, m_c, v_c, s_c, pa_c, sr_c, bo_c, li_c, signal_intelligence
    )

    # Advisory/summary layers consume the central data, never raw specialist data.
    advisory = {
        "trend": t_c, "momentum": m_c, "volatility": v_c, "structure": s_c,
        "price_action": pa_c, "sr": sr_c, "breakout": bo_c, "liquidity": li_c,
        "regime": re_c, "session": se_c, "currency_strength": ev("CurrencyStrengthEngine", {}),
        "mtf": mtf_c, "economic": eco_c, "cot": cot_c, "correlation": corr_c,
        "confluence": c, "data_quality": dq, "ema": ema_c, "macd": macd_c,
        "fvg": ev("FairValueGapEngine", {}), "microstructure": ev("MarketMicrostructureEngine", {}),
        "order_flow": ev("OrderFlowVolumeFlowEngine", {}), "reversal": ev("ReversalDetectionEngine", {}),
        "advanced_momentum": adv_m_c, "ai": ai_c, "volume_flow": volume_flow_c,
        "candle_timing": candle_timing_c, "market_region": market_region_c,
        "direct_probability": direct_probability_c,
        "news_filter": news_filter_c,
    }
    advisory["market_tracker"] = {
        "direction": c.get("direction", "WAIT"), "score": c.get("score", 0),
        "status": "CURRENT PAIR"
    }
    advisory["signal_timing"] = SignalTimingEngine.assess(
        pd.Timestamp.now(tz="UTC"), cfg.signal_max_age_seconds
    )

    ensemble = EnsembleDecisionEngine.decide(advisory, ai_c, dq, cfg)
    advisory["ensemble"] = ensemble
    no_trade = NoTradeEngine.evaluate(advisory, ensemble, dq, cfg)
    advisory["no_trade"] = no_trade
    quality = TradeQualityFilterLayer.evaluate(advisory, ensemble, no_trade, dq)

    # Risk is downstream of central evidence and the summary/intelligence path.
    risk = RiskEngine.evaluate(
        {
            "daily_loss_pct": 0,
            "drawdown_pct": 0,
            "open_positions": len([x for x in st.session_state.journal if x.get("status") == "OPEN"]),
        }, cfg, c, v_c, eco_c, corr_c,
    )
    advisory["risk"] = risk
    no_trade = NoTradeEngine.evaluate(advisory, ensemble, dq, cfg)
    advisory["no_trade"] = no_trade
    ...
    # ------------------------------------------------------------------
    # 3) ENTRY / TIMING / EXECUTION / HEALTH / SAFETY -> FINAL DECISION.
    # ------------------------------------------------------------------
    intelligence_direction = str(signal_intelligence.get("market_context", "NEUTRAL")).upper()
    entry_zone = EntryZoneAssessmentEngine.assess(df, intelligence_direction, c, cfg, symbol)
    entry_candidate = ForexEntryEngine.calculate(df, intelligence_direction, c, cfg, symbol=symbol)
    last_price = float(df.close.iloc[-1]) if not df.empty else None
    strike_validation = StrikeEntryValidationEngine.validate(entry_candidate, current_price=last_price)
    ...
    time_sync = TimeSynchronizationEngine.snapshot()
    api_connection = APIConnectionManager.status(cfg)

    entry_valid = bool(
        entry_candidate.get("approved")
        and entry_zone.get("approved")
    ...
    trade_quality_valid = bool(
        float(quality.get("score", 0.0) or 0.0) >= float(getattr(cfg, "min_score", 72.0))
        and not quality.get("hard_vetoes")
    )
    entry_zone_valid = bool(entry_zone.get("approved") and strike_validation.get("valid", False))
    news_filter_valid = str(news_filter_c.get("filter_status", "PASS")).upper() != "BLOCK"
    engine_health_valid = str(engine_health.get("status", "FAILED")).upper() != "FAILED"
    time_sync_valid = str(time_sync.get("clock_status", "UNSYNCHRONIZED")).upper() == "SYNCHRONIZED"

    gate_context = {
        "trade_quality_valid": trade_quality_valid,
        "entry_zone_valid": entry_zone_valid,
        "news_filter_valid": news_filter_valid,
        "engine_health_valid": engine_health_valid,
        "time_sync_valid": time_sync_valid,
        "central_routing_complete": central_assessment.get("routing_status") == "COMPLETE",
    }
    final_decision = FinalDecisionEngine.resolve(
        signal_intelligence,
        risk,
    ...
    result = {
        ...
        "trend": t_c, "momentum": m_c, "volatility": v_c, "structure": s_c,
        "price_action": pa_c, "sr": sr_c, "breakout": bo_c, "liquidity": li_c,
        "ema": ema_c, "macd": macd_c, "fvg": ev("FairValueGapEngine", {}),
        "microstructure": ev("MarketMicrostructureEngine", {}),
        "order_flow": ev("OrderFlowVolumeFlowEngine", {}),
        "reversal": ev("ReversalDetectionEngine", {}), "regime": re_c,
        "session": se_c, "currency_strength": ev("CurrencyStrengthEngine", {}),
        "volume_flow": volume_flow_c, "candle_timing": candle_timing_c,
        "market_region": market_region_c, "direct_probability": direct_probability_c,
        "market_tracker": advisory["market_tracker"], "signal_timing": advisory["signal_timing"],
        "mtf": mtf_c, "economic": eco_c, "news_filter": news_filter_c, "cot": cot_c,
        "correlation": corr_c, "confluence": c, "risk": risk,
        "advanced_momentum": adv_m_c, "ai": ai_c, "ensemble": ensemble,
        "no_trade": no_trade, "trade_quality": quality, "data_quality": dq,
        "engine_evidence_packages": evidence_packages,
        "context_intelligence_packages": context_packages,
        "central_assessment": central_assessment,
        "signal_intelligence": signal_intelligence,
        "central_routing": central_assessment.get("routing_map", {}),
        "forex_entry": entry_candidate,
        "entry_zone_assessment": entry_zone,
        "strike_entry_validation": strike_validation,
        "entry_validation": entry_candidate,
        "engine_health": engine_health,
        "api_connection": api_connection,
        "time_synchronization": time_sync,
        "execution_quality": exec_package,
        "final_decision": final_decision,
        "final_decision_gates": gate_context,
        "configuration_governance": ConfigurationRulesGovernanceLayer.snapshot(cfg),
        "system_circuit_breaker": SYSTEM_CIRCUIT_BREAKER.status(),
        "decision_replay": TradeJournalDecisionReplayEngine.record(final_decision, central_assessment, signal_intelligence),
        # Compatibility aliases used by earlier code.
        "t": t_c, "m": m_c, "v": v_c, "s": s_c, "pa": pa_c, "bo": bo_c, "li": li_c,
        "re": re_c, "se": se_c, "cs": ev("CurrencyStrengthEngine", {}), "eco": eco_c,
        "corr": corr_c, "c": c, "vf": volume_flow_c, "ct": candle_timing_c,
        "mr": market_region_c, "dp": direct_probability_c,
    }
    return result
...
def _v14_render_architecture_testing(df, cfg, symbol, timeframe):
    ...
    st.caption(
        "Tests the actual V14 decision path chronologically. V14 Backtest and V14 Forward Test are the authoritative testing tools."
    )

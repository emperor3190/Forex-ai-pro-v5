...
"""
V14 AI Forex Trading Bot — Accuracy, Robustness & Unified Real-Forex Data V11
...
class DerivRealForexStream:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.market_data_mode == "AUTHENTICATED":
                    ...
                    # is blocked or unavailable. It needs an App ID but no account token.
                    if self.app_id:
                        ws_candidates.append(f"wss://ws.binaryws.com/websockets/v3?app_id={self.app_id}")
    ...
    def snapshot(self):
        ...
        s["healthy"] = bool(s.get("connected") and s.get("validated") and s.get("streaming") and (s.get("authenticated") or s.get("connection_mode") == "PUBLIC"))
        return s
...
def get_deriv_real_forex_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    target_rows = max(1000, int(getattr(cfg, "historical_target_rows", cfg.deriv_stream_outputsize) or cfg.deriv_stream_outputsize))
    ...
    status = stream.start(
        wait_seconds=max(120, int(getattr(cfg, "deriv_history_timeout_seconds", 180) or 180)),
    ...
    meta = {
        ...
        "historical_target_rows": int(target_rows),
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            st.caption(
                ...
                "uses Deriv public market data first (no PAT required)."
            )

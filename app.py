...
class DerivRealForexStream:
    ...
    def _on_open(self, ws):
        with self.lock:
            self.status.update({"connected":True,"streaming":False,"error":None})
    ...
    def _on_message(self, ws, raw):
        ...
        if data.get("error"):
            with self.lock:
                self.status["error"] = data["error"].get("message", "Deriv WebSocket error")
            return
            ...
            try:
                ...
                for tf in self.TF_SECONDS:
                    self._send({"ticks_history":self.deriv_symbol,"count":self.outputsize,"end":"latest",
                                "style":"candles","granularity":self.TF_SECONDS[tf],"req_id":2000 + list(self.TF_SECONDS).index(tf)})
                self._send({"ticks":self.deriv_symbol,"subscribe":1,"req_id":3001})
            except Exception as ex:
                with self.lock: self.status["error"] = str(ex)
            return
        if typ == "candles":
            req_id = int(data.get("echo_req",{}).get("req_id",0) or 0)
            idx = req_id - 2000
    ...
    def start(self, wait_seconds=15):
        ...
        while _time.time() < deadline:
            ...
            _time.sleep(0.1)
        raise RuntimeError("Deriv real-Forex stream did not validate/bootstrap within the startup window.")

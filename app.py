...
def data_matches_selection(symbol: str, timeframe: str, df=None) -> bool:
    """True only when the currently stored dataframe belongs to this selection."""
    if df is None or df.empty:
        return False
...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    if not force and key in cache:
        ...
        if cached.get("df") is not None and not cached["df"].empty:
            return cached["df"], cached.get("meta", {})
...
def get_selected_market_data(symbol: str, timeframe: str, cfg: Config):
    ...
    if data_matches_selection(symbol, timeframe, st.session_state.get("data")):
...
def dashboard():
    ...
    with st.sidebar:
        ...
        data_source = st.selectbox("Data source", ["DEMO", "TWELVE DATA", "MT5"], index=0)
    ...
    try:
        ...
        st.error(f"Market data error for {symbol}/{timeframe}: {ex}")
    ...
    sync_ok = bool(not df.empty and loaded_symbol == selected_symbol and str(st.session_state.get("data_timeframe", "")).upper() == str(timeframe).upper())
    if sync_ok:
        st.success(f"🔗 DATA SYNC LOCKED · Dashboard {selected_symbol} = Loaded {loaded_symbol} · {timeframe}")
    else:
        st.error(f"⛔ DATA SYNC BLOCKED · Dashboard {selected_symbol}/{timeframe} does not match loaded data {loaded_symbol or 'NONE'}/{st.session_state.get('data_timeframe') or 'NONE'}")
    ...
    st.caption(
        ...
        f"Timeframe: {timeframe} · Data: {'OK' if validation['data_ok'] else 'INSUFFICIENT'} · UTC · "

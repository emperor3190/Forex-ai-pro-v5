...
class MultiTimeframeEngine:
    ...
    def _direct_live_data(df, symbol, timeframe, cfg):
        ...
        if requested_source == "DERIV REAL FOREX":
            ...
            status = stream.start(
                wait_seconds=max(30, min(120, int(getattr(cfg, "deriv_history_timeout_seconds", 60) or 60))),
                required_timeframe=str(timeframe).upper(),
                required_rows=min(int(getattr(cfg, "minimum_history_candles", 1000) or 1000), int(getattr(cfg, "deriv_stream_outputsize", 12000) or 12000)),
            )
            frame = stream.frame(str(timeframe).upper())
            if frame is None or frame.empty:
                raise RuntimeError(
                    f"Deriv public market-data stream has no {canonical_symbol(symbol)}/{str(timeframe).upper()} frame. "
                    f"Diagnostic: {status}"
                )
...
class DerivRealForexStream:
    ...
    def _send_next_history_request(self):
        with self.lock:
            if self.history_inflight is not None:
                return
            while self.history_queue and self.history_complete.get(self.history_queue[0], False):
                self.history_queue.pop(0)
            if not self.history_queue:
                self.status["history_inflight"] = None
                return
            tf = str(self.history_queue[0]).upper()
            seconds = self.TF_SECONDS[tf]
            page = int(self.history_pages.get(tf, 0))
            if page >= self.history_max_pages:
                self.history_complete[tf] = False
                self.history_errors[tf] = f"Maximum historical pages reached before target ({self.outputsize:,} candles)."
                self.history_queue.pop(0)
                self.history_inflight = None
                self.status["history_error"] = self.history_errors[tf]
                # Continue other timeframes; the required timeframe will be rejected by start().
                next_needed = True
            else:
                next_needed = False
                end = "latest" if page == 0 else (
                    int(pd.Timestamp(self.history_oldest[tf]).timestamp()) - 1
                    if self.history_oldest.get(tf) is not None else "latest"
                )
                self.history_pages[tf] = page + 1
                self.history_inflight = {"timeframe":tf, "page":page, "end":end}
                self.history_retry_count = 0
                self.status["history_inflight"] = dict(self.history_inflight)
                req_id = 2000 + list(self.TF_SECONDS).index(tf) * 100 + page
        if next_needed:
            self._send_next_history_request()
            return
        self._send({
    ...
    def _on_message(self, ws, raw):
        ...
        if data.get("error"):
            ...
            if active is not None and self.history_retry_count < 1:
                with self.lock:
                    self.history_retry_count += 1
                    tf = active["timeframe"]
                    self.history_pages[tf] = max(0, int(self.history_pages.get(tf, 1)) - 1)
                    self.history_inflight = None
                    self.status["history_inflight"] = None
                self._send_next_history_request()
            elif active is not None:
                self._finish_history_request(active["timeframe"], False, message)
            return

...
"""
V14 AI Forex Trading Bot — V12.1 Protected Baseline + Frozen Architecture
...
use candles belonging to a different currency pair. The Deriv real-Forex source uses the
...
class SignalIntelligenceLayer:
    """Integrates only evidence already accepted by Central Assessment."""
    @staticmethod
    def assess(central: Dict[str, Any], evidence_packages: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        calc_ts = utc_now_iso()
        if not central.get("assessment_readiness", False):
            return {
                ...
                "integrated_probability": None,
                "calculation_timestamp": calc_ts,
                "output_timestamp": utc_now_iso(),
                "reason": "CENTRAL_ASSESSMENT_BLOCKED",
            }

        directions = []
        scores = []
        for pkg in evidence_packages.values():
            ev = pkg.get("evidence", {})
            d = str(ev.get("direction", ev.get("bias", ""))).upper()
            if d in {"BULLISH", "BEARISH"}:
                directions.append(d)
            for key in ("score", "strength", "confidence", "quality_score"):
                if key in ev:
                    try:
                        scores.append(float(ev[key]))
                        break
                    except (TypeError, ValueError):
                        pass
        ...
        agreement = 100.0 * max(bull, bear) / max(len(directions), 1)
        score = float(np.clip(np.mean(scores) if scores else 0.0, 0, 100))
        return {
            "status": "READY",
            "symbol": central.get("symbol"),
            "timeframe": central.get("timeframe"),
            "market_context": direction,
            "evidence_summary": {"bullish": bull, "bearish": bear, "neutral": len(directions)-bull-bear},
            "engine_confluence": agreement,
            "engine_conflicts": central.get("conflict_map", []),
            "integrated_probability": score / 100.0,
            "confidence_score": score,
            "overall_intelligence_score": float((score + agreement) / 2.0),
            "decision_readiness": bool(agreement >= 50 and score >= 50),
            "calculation_timestamp": calc_ts,
            "output_timestamp": utc_now_iso(),
        }
...
class Config:
    ...
    # Legacy field retained for compatibility; authenticated mode obtains the actual URL via OTP.
    ...
    binary_trading_enabled: bool = False
...
# ============================================================
# IMPORTANT:
#   * The former legacy BacktestEngine / WalkForwardEngine have been removed.
...
def _v14_historical_daily_anchor(window: pd.DataFrame, direction: str) -> Dict[str, Any]:
    """Optional completed-D1 context from data already available in the replay window."""
    x = MarketDataEngine.normalize(window)
    if x.empty:
        return {"status": "UNAVAILABLE", "signal": "WAIT", "locked": False, "reason": "NO DATA"}
    d1 = MarketDataEngine.resample(x, "D1")
    if d1.empty:
        return {"status": "UNAVAILABLE", "signal": "WAIT", "locked": False, "reason": "NO D1 DATA"}
    # Only use D1 candles strictly before the signal day. Never use the current
    # signal day's later high/low/close to determine its own daily context.
    signal_day = pd.Timestamp(x.time.iloc[-1]).tz_convert("UTC").date()
    d1 = d1[d1.time.dt.tz_convert("UTC").dt.date < signal_day].copy()
    d1 = d1[d1.time.dt.dayofweek < 5].reset_index(drop=True)
    if len(d1) < 20:
        return {
            "status": "UNAVAILABLE", "signal": "WAIT", "locked": False,
            "reason": "INSUFFICIENT_COMPLETED_D1_CONTEXT", "rows": int(len(d1)),
        }
    t = TrendEngine.analyze(d1)
    m = MomentumEngine.analyze(d1)
    v = VolatilityEngine.analyze(d1)
    s = StructureEngine.analyze(d1)
    pa = PriceActionEngine.analyze(d1)
    sr = SupportResistanceEngine.analyze(d1)
    bo = BreakoutEngine.analyze(d1)
    li = LiquidityEngine.analyze(d1)
    dirs = [
        t.get("direction"), m.get("direction"), s.get("direction"),
        pa.get("direction"), bo.get("direction"), li.get("direction"),
    ]
    valid = [d for d in dirs if d in {"BULLISH", "BEARISH"}]
    bull, bear = valid.count("BULLISH"), valid.count("BEARISH")
    daily = "BULLISH" if bull > bear else "BEARISH" if bear > bull else "WAIT"
    return {
        "status": "LOCKED" if daily in {"BULLISH", "BEARISH"} else "UNAVAILABLE",
        "signal": daily,
        "locked": daily in {"BULLISH", "BEARISH"},
        "aligned_with_candidate": daily == direction,
        "rows": int(len(d1)),
        "candidate_direction": direction,
        "timestamp": str(d1.time.iloc[-1]),
    }
...
def _v14_historical_trade_quality(a: Dict[str, Any], ensemble: Dict[str, Any],
    ...
    """Historical-test adapter for the existing Trade Quality responsibility.

    The production TradeQualityEngine has live-only hard gates for a currently
    locked D1 candle and current-market MTF. A historical replay cannot truthfully
    label those as current. This adapter therefore evaluates the same evidence
    categories with historical availability explicitly reported instead of
    pretending that live data exists.
    """
    mtf = a.get("mtf", {}) or {}
    ...
    coverage = 100.0 * len(usable) / max(len(MultiTimeframeEngine.TIMEFRAMES), 1)
    ...
    # High-accuracy objective: a candidate must clear a stronger quality gate.
    if score < 80:
        vetoes.append("TRADE QUALITY BELOW 80")
...
class V14ReplayDecisionEngine:
    ...
    def evaluate(window: pd.DataFrame, symbol: str, timeframe: str,
        ...
        direction = str(intelligence.get("market_context", "NEUTRAL")).upper()
        entry = ForexEntryEngine.calculate(x, direction, c, cfg, symbol=symbol)
        ...
        historical_tq = _v14_historical_trade_quality(advisory, ensemble, no_trade, dq, direction)
...
class V14BacktestEngine:
    ...
    @staticmethod
    def run(df: pd.DataFrame, cfg: Config, symbol="EURUSD", timeframe="M5"):
        x = MarketDataEngine.normalize(df)
        if len(x) < V14BacktestEngine.MIN_CANDLES:
            return pd.DataFrame(), {
                "error": f"V14 Backtest requires at least {V14BacktestEngine.MIN_CANDLES} candles in the COMPLETE dataset.",
                "rows": int(len(x)), "required": V14BacktestEngine.MIN_CANDLES,
            }
        ...
        for i in range(V14BacktestEngine.WARMUP, len(x) - 1):
            ...
            if ai_cache is None or i - ai_cache_bar >= V14BacktestEngine.AI_REFRESH_BARS:
                ai_cache = AIProbabilityEngine.predict(window)
                ai_cache_bar = i
...
class V14ForwardTestingEngine:
    ...
    def run(df: pd.DataFrame, cfg: Config, symbol="EURUSD", timeframe="M5", train_ratio=0.65):
        ...
        test_df = x.copy()
        bt_train, train_metrics = V14BacktestEngine.run(train_df, cfg, symbol, timeframe)
        ...
        for i in range(cut, len(x) - 1):
            ...
            if ai_cache is None or i - ai_cache_bar >= V14BacktestEngine.AI_REFRESH_BARS:
                ...
                ai_cache_bar = i
            replay=V14ReplayDecisionEngine.evaluate(window,symbol,timeframe,cfg,equity,ai_override=ai_cache)
            if not replay.get("eligible"): continue
...
class FullStackDailySignalEngine:
    """Full-stack D1 calculator for both completed and forming daily states."""
    EXCLUDED_FROM_DAILY_VOTE = {"daily_signal"}

    @classmethod
    def _ledger(cls, analysis, data_quality, locked_context=None):
        bus = EngineInformationBus.build(analysis, data_quality, daily_signal=locked_context)
        rows=[]; weighted=0.0; vote_weight=0.0; healthy=0; available=0
        for label,key,role,weight in EngineInformationBus.SPECS:
            if key in cls.EXCLUDED_FROM_DAILY_VOTE: continue
            item=bus.get(label,{})
            direction=item.get("direction","NEUTRAL")
            strength=float(item.get("strength",0.0) or 0.0)
            ok=bool(item.get("available")) and bool(item.get("healthy"))
            available += int(bool(item.get("available"))); healthy += int(ok)
            directional=direction in {"BULLISH","BEARISH"}
            signed=1.0 if direction=="BULLISH" else -1.0 if direction=="BEARISH" else 0.0
            contribution=0.0
            if directional and ok:
                scale=max(0.25,min(1.0,strength/100.0)) if strength else 0.5
                contribution=signed*float(weight)*scale
                weighted+=contribution; vote_weight+=float(weight)*scale
            rows.append({"engine":label,"key":key,"role":role,"weight":float(weight),"direction":direction,
                         "strength":round(strength,2),"healthy":ok,"available":bool(item.get("available")),
                         "directional_vote":bool(directional and ok),"contribution":round(contribution,5),
                         "status":"HEALTHY" if ok else "UNAVAILABLE/DEGRADED"})
        normalized=weighted/max(vote_weight,1e-9)
        direction="BULLISH" if normalized>0.10 else "BEARISH" if normalized<-0.10 else "WAIT"
        agreement=abs(weighted)/max(vote_weight,1e-9)*100.0
        participation=healthy/max(len(rows),1)*100.0
        return {"signal":direction,"weighted_score":round(weighted,5),"normalized_score":round(normalized,5),
                "directional_agreement":round(agreement,1),"engine_participation":round(participation,1),
                "available_engines":available,"healthy_engines":healthy,"total_engines":len(rows),
                "ledger":rows,"bus":bus}

    @classmethod
    def evaluate_completed(cls, df, symbol, cfg):
        completed=DailySignalAssessmentEngine.completed_d1(df)
        if completed.empty: return {"status":"UNAVAILABLE","signal":"WAIT","locked":False,"reason":"NO COMPLETED D1 CANDLE"}
        dq=dict(DataIntegrityEngine.assess(completed,"D1",max(int(cfg.data_max_age_seconds),432000)))
        dq["signal_allowed"]=dq.get("score",0)>=70 and not any("INVALID" in r for r in dq.get("reasons",[]))
        analysis=analyze_market(completed,canonical_symbol(symbol),_daily_analysis_config(cfg),"D1",deep_mtf=False)
        ledger=cls._ledger(analysis,dq)
        reasons=[]
        if not dq["signal_allowed"]: ledger["signal"]="WAIT"; reasons.append("DAILY DATA QUALITY FAILED")
        if ledger["engine_participation"]<70: ledger["signal"]="WAIT"; reasons.append("INSUFFICIENT FULL-STACK ENGINE PARTICIPATION")
        risk=analysis.get("risk") or {}
        if isinstance(risk,dict) and not risk.get("approved",True): ledger["signal"]="WAIT"; reasons.append("DAILY RISK GATE")
        conf=min(float(ledger["directional_agreement"]),float(ledger["engine_participation"]),float(dq.get("score",0)))
        locked=bool(dq["signal_allowed"] and ledger["engine_participation"]>=70)
        return {"status":"LOCKED" if locked else "UNCONFIRMED","signal":ledger["signal"],"locked":locked,
                "cycle_utc":datetime.now(timezone.utc).date().isoformat(),"completed_candle":str(completed["time"].iloc[-1]),
                "confidence":round(conf,1),"engine_agreement":ledger["directional_agreement"],"engine_participation":ledger["engine_participation"],
                "weighted_score":ledger["weighted_score"],"normalized_score":ledger["normalized_score"],
                "healthy_engines":ledger["healthy_engines"],"total_engines":ledger["total_engines"],"available_engines":ledger["available_engines"],
                "reasons":reasons,"data_quality":dq,"engine_contributions":ledger["ledger"],"engine_bus":ledger["bus"],"analysis":analysis}

    @classmethod
    def evaluate_forming(cls, df, symbol, cfg, locked=None):
        x=_v13_forming_daily_frame(df)
        if x.empty: return {"status":"UNAVAILABLE","signal":"WAIT","locked":False,"reason":"NO FORMING D1 DATA"}
        dq=dict(DataIntegrityEngine.assess(x,"D1",max(int(cfg.data_max_age_seconds),432000)))
        dq["signal_allowed"]=dq.get("score",0)>=60 and not any("INVALID" in r for r in dq.get("reasons",[]))
        analysis=analyze_market(x,canonical_symbol(symbol),_daily_analysis_config(cfg),"D1",deep_mtf=False)
        ledger=cls._ledger(analysis,dq,locked_context=locked)
        conf=min(float(ledger["directional_agreement"]),float(ledger["engine_participation"]),float(dq.get("score",0)))
        ls=str((locked or {}).get("signal","WAIT")).upper(); fs=ledger["signal"]
        alignment="CONFIRMING" if fs in {"BULLISH","BEARISH"} and fs==ls else "CONTRADICTING" if fs in {"BULLISH","BEARISH"} and ls in {"BULLISH","BEARISH"} else "NO_CONFIRMED_THESIS"
        return {"status":"LIVE","signal":fs,"locked":False,"cycle_utc":datetime.now(timezone.utc).date().isoformat(),
                "confidence":round(conf,1),"engine_agreement":ledger["directional_agreement"],"engine_participation":ledger["engine_participation"],
                "healthy_engines":ledger["healthy_engines"],"total_engines":ledger["total_engines"],"available_engines":ledger["available_engines"],
                "alignment":alignment,"data_quality":dq,"engine_contributions":ledger["ledger"],"engine_bus":ledger["bus"],"analysis":analysis}


class CurrentDailyMarketSentimentEngine:
    """Live D1 market-sentiment layer built from the complete V13 engine ledger.

    This is intentionally separate from the immutable Daily Signal. It evaluates the
    currently forming D1 candle and every available V13 engine contribution, then
    exposes a human-readable sentiment state for the dashboard and central quality
    layer. It never writes to or replaces the persistent Daily Signal lock.
    """

    @staticmethod
    def _candle_context(df: pd.DataFrame) -> Dict[str, Any]:
        if df is None or df.empty:
            return {"state": "UNKNOWN", "body_pct": 0.0, "range_pct": 0.0, "close_location": 50.0}
        x = MarketDataEngine.normalize(df)
        last = x.iloc[-1]
        try:
            o, h, lo, c = map(float, [last["open"], last["high"], last["low"], last["close"]])
            rng = max(h - lo, 1e-12)
            body = c - o
            return {
                "state": "BULLISH" if body > 0 else "BEARISH" if body < 0 else "NEUTRAL",
                "body_pct": round(abs(body) / rng * 100.0, 1),
                "range_pct": round(rng / max(abs(c), 1e-12) * 100.0, 4),
                "close_location": round((c - lo) / rng * 100.0, 1),
    ...
    @classmethod
    def evaluate(cls, forming_report: Dict[str, Any], locked_report: Dict[str, Any] | None = None) -> Dict[str, Any]:
        forming_report = forming_report or {}
        ledger = list(forming_report.get("engine_contributions", []) or [])
        directional = [r for r in ledger if r.get("directional_vote")]
        weighted_score = float(forming_report.get("weighted_score", 0.0) or 0.0)
        normalized = float(forming_report.get("normalized_score", 0.0) or 0.0)
        participation = float(forming_report.get("engine_participation", 0.0) or 0.0)
        agreement = float(forming_report.get("engine_agreement", forming_report.get("directional_agreement", 0.0)) or 0.0)

        # All published V13 engine rows are retained in the sentiment ledger. Directional
        # engines vote; context/risk/data engines contribute through health/participation.
        bull_weight = sum(float(r.get("weight", 0.0)) * max(float(r.get("strength", 0.0)) / 100.0, 0.25)
                          for r in directional if str(r.get("direction", "")).upper() == "BULLISH")
        bear_weight = sum(float(r.get("weight", 0.0)) * max(float(r.get("strength", 0.0)) / 100.0, 0.25)
                          for r in directional if str(r.get("direction", "")).upper() == "BEARISH")
        total_directional = bull_weight + bear_weight
        directional_bias = "BULLISH" if bull_weight > bear_weight else "BEARISH" if bear_weight > bull_weight else "NEUTRAL"

        # Every engine affects the sentiment quality calculation. Directional engines
        # determine the bias; context/risk/timing/data engines affect the reliability
        # of that bias through the full-stack health factor.
        all_rows = [r for r in ledger if r.get("available")]
        if all_rows:
            quality_weight = sum(float(r.get("weight", 0.0) or 0.0) for r in all_rows)
            quality_score = (sum(float(r.get("weight", 0.0) or 0.0) * (100.0 if r.get("healthy") else 0.0) for r in all_rows)
                             / max(quality_weight, 1e-9))
        else:
            quality_score = 0.0
        ...
        # Confidence is deliberately capped by engine participation and data quality.
        dq = forming_report.get("data_quality", {}) or {}
        dq_score = float(dq.get("score", 0.0) or 0.0)
        confidence = min(100.0, max(0.0, (abs(normalized) * 100.0 * 0.45) + agreement * 0.20 + participation * 0.15 + quality_score * 0.20))
        confidence = min(confidence, dq_score if dq_score else confidence, participation if participation else confidence, quality_score if quality_score else confidence)

        lock = (locked_report or {}).get("signal", "WAIT")
        lock = str(lock).upper()
        relation = "NO_LOCK"
        if directional_bias in {"BULLISH", "BEARISH"} and lock in {"BULLISH", "BEARISH"}:
            relation = "CONFIRMING" if directional_bias == lock else "CONTRADICTING"
        ...
        return {
            ...
            "weighted_score": round(weighted_score, 5),
            "engine_agreement": round(agreement, 1),
            "engine_participation": round(participation, 1),
            "healthy_engines": int(forming_report.get("healthy_engines", 0) or 0),
            "total_engines": int(forming_report.get("total_engines", len(ledger)) or len(ledger)),
            "available_engines": int(forming_report.get("available_engines", 0) or 0),
            "bullish_weight": round(bull_weight, 4),
            "bearish_weight": round(bear_weight, 4),
            "directional_engines": len(directional),
            "full_stack_quality": round(quality_score, 1),
            "all_engine_contributions": ledger,
            "data_quality": dq,
            "locked_daily_signal": lock,
            "relation_to_lock": relation,
            "updated_utc": datetime.now(timezone.utc).isoformat(),
            "note": "Current D1 sentiment is live and may change; it never rewrites the locked Daily Signal.",
        }


class PersistentDailyLockStore:
    """SQLite-backed lock store; session_state is only a fast cache."""
    DB_PATH=os.environ.get("V13_DAILY_LOCK_DB",os.path.join(os.getcwd(),"v13_daily_signal_lock.sqlite3"))
    @classmethod
    def _conn(cls):
        conn=sqlite3.connect(cls.DB_PATH,timeout=10)
        conn.execute("CREATE TABLE IF NOT EXISTS daily_locks (lock_key TEXT PRIMARY KEY, cycle_utc TEXT, symbol TEXT, source TEXT, payload TEXT, created_utc TEXT)");conn.commit();return conn
    @classmethod
    def get(cls,key):
        try:
            with cls._conn() as c: row=c.execute("SELECT payload FROM daily_locks WHERE lock_key=?",(key,)).fetchone()
    ...
    def put(cls,key,payload):
        try:
            with cls._conn() as c:
                c.execute("INSERT OR REPLACE INTO daily_locks(lock_key,cycle_utc,symbol,source,payload,created_utc) VALUES(?,?,?,?,?,?)",
                          (key,payload.get("cycle_utc",""),payload.get("symbol",""),payload.get("source",""),json.dumps(payload,default=str),datetime.now(timezone.utc).isoformat()));c.commit()
        except Exception as exc: st.session_state["daily_lock_persistence_error"]=str(exc)
...
def _attach_daily_context(analysis: Dict[str, Any], daily_context=None) -> Dict[str, Any]:
    """Expose the immutable locked Daily Signal as shared context to every engine output.

    Engines remain free to disagree; this context is never used to overwrite their
    own calculation. It lets downstream diagnostics and Trade Quality prove whether
    each engine confirms, conflicts with, or is neutral to the locked daily anchor.
    """
    if not isinstance(analysis, dict) or not isinstance(daily_context, dict):
        return analysis
    locked = str(daily_context.get("signal", "WAIT")).upper()
    if locked not in {"BULLISH", "BEARISH"}:
        return analysis
    for key, value in list(analysis.items()):
        if isinstance(value, dict) and key not in {"data_quality", "risk", "risk_veto", "daily_signal"}:
            d = str(value.get("direction", value.get("bias", "NEUTRAL"))).upper()
            value["daily_locked_signal"] = locked
            value["daily_alignment"] = (
                "CONFIRMING" if d == locked else
                "CONTRADICTING" if d in {"BULLISH", "BEARISH"} else
                "NEUTRAL"
            )
    analysis["daily_context"] = {
        "signal": locked,
        "cycle_utc": daily_context.get("cycle_utc"),
        "locked": bool(daily_context.get("locked")),
        "confidence": daily_context.get("confidence", 0),
    }
    return analysis
...
def analyze_market(df, symbol, cfg, timeframe=None, *, deep_mtf=True):
    ...
    # If a validated lock is already available from the previous daily cycle,
    # expose it as immutable context to all engine outputs. It does not rewrite
    # any engine's own direction.
    daily_context = st.session_state.get("daily_signal_locked")
    result = {
        "trend": t,
        "momentum": m,
        "volatility": v,
    ...
    # and published to Central Assessment. Existing result keys remain unchanged
    ...
    for engine_name, evidence in evidence_inputs.items():
        evidence_packages[engine_name] = package_engine_evidence(
            ...
            quality={
                ...
                "status": "PASS" if dq.get("signal_allowed", False) else "BLOCK",
            },
    ...
    result = _attach_daily_context(result, daily_context)
    return result
...
def warm_deriv_multi_pair_streams(symbols, cfg):
    ...
    supplies all V13 timeframes (M1 through D1). Existing healthy streams are
...
def build_full_pair_signal_snapshot(sym, x, cfg, timeframe):
    """Run the complete V13 intelligence/gating stack for one live pair.

    This is deliberately separate from the dashboard renderer. The scanner gets
    the same technical engines, MTF, D1 lock, forming-D1 sentiment, confluence,
    ensemble, risk, no-trade and trade-quality layers used by the selected pair.
    """
    aa = analyze_market(x, sym, cfg, timeframe, deep_mtf=True)

    # Each pair owns its own immutable UTC-day Daily Signal.
    daily = get_locked_daily_signal(sym, cfg, force=False)

    # The forming D1 calculation is live and cannot rewrite the lock.
    forming = get_forming_daily_signal(sym, cfg, locked=daily, force=False)
    sentiment = CurrentDailyMarketSentimentEngine.evaluate(forming, daily)

    # Inject the pair's locked D1 signal into the central trade-quality layer.
    refresh_trade_quality_with_daily_signal(aa, daily)
    ...
    final_direction = str(
        aa.get("final_decision", {}).get(
            "final_decision",
            "NO-TRADE",
        )
    ).upper()

    # A scanner row may display a final decision only when the V14 final
    # authority says BUY/SELL and the retained eligibility stack also passes.
    signal = final_direction if (
        live_gate.get("eligible") and final_direction in {"BUY", "SELL"}
    ) else "NO-TRADE"
...
# ============================================================
# V13 ADDITION: DERIV AUTHENTICATED REAL-FOREX LIVE STREAM (READ-ONLY)
# ============================================================
class DerivRealForexStream:
    ...
    def _validate_active_symbols(self, rows):
        """Compatibility validator retained from the earlier V13 adapter.
        ...
        if typ == "candles":
            ...
            if 0 <= idx < len(tfs):
                ...
                if not frame.empty:
                    ...
                    with self.lock:
                        self.frames[tf] = MarketDataEngine.normalize(frame).tail(self.outputsize).reset_index(drop=True)
            return
...
def get_locked_daily_signal(symbol: str, cfg: Config, force: bool = False):
    """Immutable full-stack Daily Signal for the current UTC day."""
    cycle=datetime.now(timezone.utc).date().isoformat(); key=f"{canonical_symbol(symbol)}|{cycle}|{str(cfg.data_source).upper()}"
    cache=st.session_state.setdefault("daily_signal_cache",{})
    if not force:
        persisted=PersistentDailyLockStore.get(key)
        if persisted:
            cache[key]=persisted; st.session_state["daily_signal_locked"]=persisted; return persisted
        if key in cache:return cache[key]
    daily_df,daily_meta=get_daily_market_data(symbol,cfg,force=force)
    report=FullStackDailySignalEngine.evaluate_completed(daily_df,symbol,cfg)
    report["source"]=str(daily_meta.get("source",cfg.data_source)).upper(); report["symbol"]=canonical_symbol(symbol); report["lock_key"]=key
    if report.get("locked"):
        cache[key]=report; st.session_state["daily_signal_locked"]=report; PersistentDailyLockStore.put(key,report)
    return report

def get_forming_daily_signal(symbol: str, cfg: Config, locked=None, force: bool = False):
    """Continuously calculate the forming D1 signal; never mutates the lock."""
    daily_df,daily_meta=get_daily_market_data(symbol,cfg,force=force)
    locked=locked or st.session_state.get("daily_signal_locked") or {}
    report=FullStackDailySignalEngine.evaluate_forming(daily_df,symbol,cfg,locked=locked)
    report["source"]=str(daily_meta.get("source",cfg.data_source)).upper(); report["symbol"]=canonical_symbol(symbol); report["locked_signal"]=locked.get("signal","WAIT")
    return report
...
def init_state():
    ...
    defaults = {
        ...
        "cot": pd.DataFrame(),
        "mc": {},
        # V14 architecture-level testing state.
        ...
        "forming_daily_signal": None,
        "daily_lock_persistence_error": None,
        "td_request_log": [],
...
def legacy_v13_dashboard():
    st.set_page_config(
        page_title="V13 AI Trading Platform · Legacy Compatibility",
        page_icon="📈",
        layout="wide",
    )
    init_state()
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            st.caption("Deriv authenticated Options WebSocket · read-only real-Forex market data. PAT credentials are read only from Streamlit Secrets; no order/execution API is called.")
            ...
            deriv_app_configured = bool(DerivRealForexStream._secret("DERIV_APP_ID"))
            deriv_token_configured = bool(DerivRealForexStream._secret("DERIV_API_TOKEN"))
            st.write(f"PAT App ID configured: {'Yes' if deriv_app_configured else 'No'}")
            st.write(f"PAT token configured: {'Yes' if deriv_token_configured else 'No'}")
            cfg.deriv_options_account_id = st.text_input(
                "Options account ID (optional — auto-discovered when blank)",
                value=st.session_state.get("deriv_options_account_id", ""),
                help="Leave blank to automatically select the first active demo Options account returned by Deriv."
            ).strip()
            st.session_state.deriv_options_account_id = cfg.deriv_options_account_id
            if st.button("🟢 Connect Authenticated Deriv Real-Forex Stream", use_container_width=True):
                try:
                    ...
                    st.success(f"Deriv authenticated real-Forex stream: {display_symbol(symbol)} · {timeframe} · {len(live_df):,} candles loaded.")
                except Exception as ex:
                    ...
                    st.error(f"Deriv authenticated real-Forex stream error: {ex}")
        else:
    ...
    # Establish/recover the immutable daily lock BEFORE intraday analysis so every
    # engine receives the locked daily context during its normal calculation.
    daily_bootstrap = None
    if not df.empty:
        try:
            daily_bootstrap = get_locked_daily_signal(symbol, cfg)
        except Exception as daily_bootstrap_ex:
            st.session_state["daily_lock_bootstrap_error"] = str(daily_bootstrap_ex)
    analysis_error = None
    if not df.empty:
        try:
    ...
    if daily_report:
        ...
        ds10, ds11, ds12 = st.columns(3)
        ds10.metric("Engine Participation", f"{daily_report.get('engine_participation', 0):.1f}%")
        ds11.metric("Healthy Engines", f"{daily_report.get('healthy_engines', 0)}/{daily_report.get('total_engines', 0)}")
        ds12.metric("Lock", "🔒 PERSISTENT" if daily_report.get('locked') else "WAITING")
    ...
    # ========================================================
    # FORMING D1 — full-stack calculation, never rewrites the lock
    # ========================================================
    forming_report = {}
    forming_error = None
    try:
        forming_report=get_forming_daily_signal(symbol,cfg,locked=daily_report)
        st.subheader("🟡 Forming D1 Signal · Full V13 Stack")
        fs1,fs2,fs3,fs4,fs5=st.columns(5)
        fs1.metric("FORMING D1",forming_report.get("signal","WAIT")); fs2.metric("Confidence",f"{forming_report.get('confidence',0):.1f}%")
        fs3.metric("Engine Participation",f"{forming_report.get('engine_participation',0):.1f}%"); fs4.metric("Engines",f"{forming_report.get('healthy_engines',0)}/{forming_report.get('total_engines',0)}")
        fs5.metric("Relation to Lock",forming_report.get("alignment","UNKNOWN"))
        if daily_report and daily_report.get("locked"):
            st.info(f"🔒 Locked Daily Signal remains {daily_report.get('signal')} for UTC day {daily_report.get('cycle_utc')}. Forming D1 cannot rewrite it.")
        with st.expander("Daily engine contribution ledger",expanded=False): st.dataframe(pd.DataFrame((daily_report or {}).get("engine_contributions",[])),use_container_width=True,hide_index=True)
        with st.expander("Forming D1 engine contribution ledger",expanded=False): st.dataframe(pd.DataFrame(forming_report.get("engine_contributions",[])),use_container_width=True,hide_index=True)

        # ========================================================
        # CURRENT DAILY MARKET SENTIMENT — live forming-D1 synthesis
        # ========================================================
        sentiment_report = CurrentDailyMarketSentimentEngine.evaluate(forming_report, daily_report)
        st.session_state["current_daily_market_sentiment"] = sentiment_report
        st.subheader("📈 Current Daily Market Sentiment · Live D1 Movement")
        sm1, sm2, sm3, sm4, sm5 = st.columns(5)
        sm1.metric("CURRENT SENTIMENT", sentiment_report.get("sentiment", "NEUTRAL / MIXED"))
        sm2.metric("Direction", sentiment_report.get("direction", "NEUTRAL"))
        sm3.metric("Confidence", f"{sentiment_report.get('confidence', 0):.1f}%")
        sm4.metric("Engine Participation", f"{sentiment_report.get('engine_participation', 0):.1f}%")
        sm5.metric("Relation to Lock", sentiment_report.get("relation_to_lock", "NO_LOCK"))
        st.caption(
            f"Live forming D1 sentiment · {display_symbol(symbol)}/D1 · "
            f"{sentiment_report.get('healthy_engines',0)}/{sentiment_report.get('total_engines',0)} healthy engines · "
            f"{sentiment_report.get('available_engines',0)}/{sentiment_report.get('total_engines',0)} available · "
            "all V13 engine outputs are retained in the contribution ledger."
        )
        if sentiment_report.get("relation_to_lock") == "CONTRADICTING":
            st.warning(
                f"⚠️ Current D1 sentiment is {sentiment_report.get('sentiment')} while the locked Daily Signal is "
                f"{sentiment_report.get('locked_daily_signal')}. The live sentiment may change, but the locked signal is not rewritten."
            )
        elif sentiment_report.get("relation_to_lock") == "CONFIRMING":
            st.success(
                f"✅ Current D1 sentiment confirms the locked Daily Signal: {sentiment_report.get('locked_daily_signal')}."
            )
        with st.expander("View complete Current Daily Sentiment engine contribution", expanded=False):
            st.json({
                "Sentiment": sentiment_report.get("sentiment"),
                "Direction": sentiment_report.get("direction"),
                "Confidence": sentiment_report.get("confidence"),
                "Engine Agreement": sentiment_report.get("engine_agreement"),
                "Engine Participation": sentiment_report.get("engine_participation"),
                "Bullish Weight": sentiment_report.get("bullish_weight"),
                "Bearish Weight": sentiment_report.get("bearish_weight"),
                "Relation to Locked Signal": sentiment_report.get("relation_to_lock"),
                "Data Quality": sentiment_report.get("data_quality"),
            })

    except Exception as forming_ex:
        forming_error = str(forming_ex)
        st.warning(f"Forming D1 signal unavailable: {forming_ex}")

    # ========================================================
    # CURRENT DAILY MARKET SENTIMENT — ALWAYS VISIBLE
    # This panel is deliberately outside the forming-D1 try/except so a failure
    # in another dashboard component cannot silently hide the sentiment section.
    # ========================================================
    st.divider()
    st.subheader("📊 Current Daily Market Sentiment")
    if forming_report:
        try:
            sentiment_report = CurrentDailyMarketSentimentEngine.evaluate(forming_report, daily_report)
            st.session_state["current_daily_market_sentiment"] = sentiment_report
            sm1, sm2, sm3, sm4, sm5 = st.columns(5)
            sm1.metric("CURRENT D1 SENTIMENT", sentiment_report.get("sentiment", "NEUTRAL / MIXED"))
            sm2.metric("D1 Direction", sentiment_report.get("direction", "NEUTRAL"))
            sm3.metric("Confidence", f"{sentiment_report.get('confidence', 0):.1f}%")
            sm4.metric("Engine Participation", f"{sentiment_report.get('engine_participation', 0):.1f}%")
            sm5.metric("Relation to Lock", sentiment_report.get("relation_to_lock", "NO_LOCK"))
            st.caption(
                f"Live current D1 movement · {display_symbol(symbol)}/D1 · "
                f"{sentiment_report.get('healthy_engines',0)}/{sentiment_report.get('total_engines',0)} healthy · "
                f"{sentiment_report.get('available_engines',0)}/{sentiment_report.get('total_engines',0)} available · "
                "forming D1 sentiment may change and NEVER rewrites the locked Daily Signal."
            )
            relation = sentiment_report.get("relation_to_lock")
            if relation == "CONTRADICTING":
                st.warning(
                    f"⚠️ Current D1 sentiment is {sentiment_report.get('sentiment')} while the locked Daily Signal is "
                    f"{sentiment_report.get('locked_daily_signal')}. The locked signal remains unchanged."
                )
            elif relation == "CONFIRMING":
                st.success(f"✅ Current D1 sentiment confirms the locked Daily Signal: {sentiment_report.get('locked_daily_signal')}.")
            with st.expander("View ALL Current D1 Sentiment Engine Contributions", expanded=False):
                rows = sentiment_report.get("all_engine_contributions", []) or []
                if rows:
                    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
                else:
                    st.info("No engine contribution rows are available yet.")
        except Exception as sentiment_ex:
            st.error(f"Current Daily Market Sentiment calculation error: {sentiment_ex}")
    else:
        st.warning("Current Daily Market Sentiment is waiting for the forming-D1 full-stack calculation.")
        if forming_error:
            st.caption(f"Forming-D1 diagnostic: {forming_error}")
    ...
    with tabs[0]:
        ...
        analyses = {}
        scanner_rows = []
        source_name = str(cfg.data_source).upper()

        if source_name == "DERIV REAL FOREX":
            # Unlike the old rotation design, every watchlist pair is brought
            # online from the authenticated Deriv stream. No pair is allowed to
            # become NO DATA merely because it was not the selected dashboard pair.
            stream_results = warm_deriv_multi_pair_streams(watch, cfg)

            for sym in watch:
                try:
                    if not stream_results.get(sym, {}).get("ok"):
                        err = stream_results.get(sym, {}).get("status", {}).get(
                            "error", "DERIV STREAM START FAILED"
                        )
                        raise RuntimeError(err)

                    x, meta = get_deriv_real_forex_data(
                        sym, timeframe, cfg, force=False
                    )
                    if x is None or x.empty:
                        raise RuntimeError("empty Deriv feed")

                    analyses[sym] = build_full_pair_signal_snapshot(
                        sym, x, cfg, timeframe
                    )
                    analyses[sym]["meta"] = meta
                except Exception as ex:
                    analyses[sym] = {
                        "_error": str(ex),
                        "analysis": {
                            ...
                            "risk": {"status": "NO DATA"},
                            "data_quality": {"score": 0.0, "status": "BAD"},
                        },
                        "signal": "NO DATA",
                        "raw_direction": "WAIT",
                        "daily": {"signal": "WAIT", "status": "UNAVAILABLE"},
                        "forming": {"signal": "WAIT"},
                        "sentiment": {
                            "sentiment": "UNAVAILABLE",
                            "direction": "NEUTRAL",
                            "confidence": 0.0,
                            "engine_participation": 0.0,
                        },
                        "live_gate": {"eligible": False, "reasons": [str(ex)]},
                        "mtf_alignment": 0.0,
                        "mtf_coverage": 0.0,
                        "data_quality": {"score": 0.0, "status": "BAD"},
                        "trade_quality": {"score": 0.0, "grade": "D", "decision": "NO TRADE"},
                    }
        else:
            # Preserve the previous protected behavior for Twelve Data / MT5 /
            # DEMO: do not increase provider usage simply because the Deriv
            # multi-pair architecture is enabled.
            scanner_idx = int(
                st.session_state.get("scanner_rotation_index", 0)
            ) % max(len(watch), 1)
            rotation_symbol = watch[scanner_idx] if watch else None
            st.session_state.scanner_rotation_index = (
                scanner_idx + 1
            ) % max(len(watch), 1)
            ...
            for sym in watch:
                try:
                    if canonical_symbol(sym) == canonical_symbol(symbol):
                        ...
                        cached = (
                            _td_cached_pair(sym, timeframe, cfg)
                            if source_name == "TWELVE DATA"
                            else None
                        )
                        if cached is not None:
                            ...
                            sym == rotation_symbol
                            and source_name == "TWELVE DATA"
                        ):
                            x, _ = get_live_pair_data(
                                sym, timeframe, cfg, force=False
                            )
                        else:
                            raise RuntimeError(
                                "NO FRESH SCANNER CACHE — ROTATION PENDING"
                            )
                    if x is None or x.empty:
                        raise RuntimeError("empty feed")

                    aa = analyze_market(
                        x, sym, cfg, timeframe, deep_mtf=False
                    )
                    analyses[sym] = {
                        "analysis": aa,
                        "daily": {"signal": "UNAVAILABLE", "status": "NOT_SCANNED"},
                        "forming": {"signal": "UNAVAILABLE"},
                        "sentiment": {
                            "sentiment": "NOT SCANNED",
                            "direction": "NEUTRAL",
                            "confidence": 0.0,
                            "engine_participation": 0.0,
                        },
                        "live_gate": {"eligible": False, "reasons": []},
                        "signal": "ANALYSIS ONLY",
                        "raw_direction": str(
                            aa.get("confluence", {}).get("direction", "WAIT")
                        ).upper(),
                        "mtf_alignment": float(
                            aa.get("mtf", {}).get("alignment", 0.0) or 0.0
                        ),
                        "mtf_coverage": float(
                            aa.get("mtf", {}).get("coverage_percent", 0.0) or 0.0
                        ),
                        "data_quality": aa.get("data_quality", {}),
                        "trade_quality": aa.get("trade_quality", {}),
                    }
                    ...
                    analyses[sym] = {
                        ...
                        "daily": {"signal": "WAIT", "status": "UNAVAILABLE"},
                        "forming": {"signal": "WAIT"},
                        "sentiment": {"sentiment": "UNAVAILABLE", "direction": "NEUTRAL", "confidence": 0.0, "engine_participation": 0.0},
                        "live_gate": {"eligible": False, "reasons": [str(ex)]},
                        "mtf_alignment": 0.0,
                        "mtf_coverage": 0.0,
                        "data_quality": {"score": 0.0, "status": "BAD"},
        ...
        for sym in watch:
            item = analyses[sym]
            aa = item.get("analysis", {})
            daily = item.get("daily", {}) or {}
            sentiment = item.get("sentiment", {}) or {}
            tq = item.get("trade_quality", {}) or {}
            dq = item.get("data_quality", {}) or {}
            gate = item.get("live_gate", {}) or {}

            if item.get("_error"):
                scanner_rows.append({
                    "SYMBOL": sym,
                    "SIGNAL": "NO DATA",
                    "D1 LOCK": str(daily.get("signal", "WAIT")),
                    "D1 SENTIMENT": sentiment.get("sentiment", "UNAVAILABLE"),
                    "MTF": "0%",
                    "ENGINE AGREEMENT": "0%",
                    "TRADE QUALITY": "0/D",
                    "DATA": dq.get("status", "BAD"),
                    "STATUS": "BLOCKED",
                    "REASON": str(item.get("_error"))[:160],
                })
                continue
            ...
            scanner_rows.append({
                ...
                "D1 LOCK": daily.get("signal", "WAIT"),
                "D1 SENTIMENT": sentiment.get("sentiment", "NEUTRAL / MIXED"),
                "MTF": f"{item.get('mtf_alignment', 0.0):.0f}%"
                      f" / {item.get('mtf_coverage', 0.0):.0f}% cov",
                "ENGINE AGREEMENT": f"{tq.get('directional_agreement', 0.0):.0f}%",
                "TRADE QUALITY": f"{tq.get('score', 0.0):.1f}/{tq.get('grade', 'D')}",
                "DATA": f"{dq.get('score', 0.0):.0f} {dq.get('status', 'UNKNOWN')}",
                "STATUS": "APPROVED" if gate.get("eligible") else "WAIT/BLOCKED",
                "REASON": " | ".join(gate.get("reasons", [])[:2]),
            })
        ...
        if not scanner.empty:
            signal_order = {
                "BUY": 0, "SELL": 0, "WAIT": 1,
                "ANALYSIS ONLY": 2, "NO DATA": 3
            }
            scanner["_order"] = scanner["SIGNAL"].map(signal_order).fillna(4)
            scanner = (
                scanner.sort_values(["_order", "TRADE QUALITY"], ascending=[True, False])
                .drop(columns=["_order"])
                .reset_index(drop=True)
            )
            if "SYMBOL" in scanner.columns:
        ...
        if source_name == "DERIV REAL FOREX":
            approved = (
                scanner[
                    scanner["STATUS"].astype(str).str.upper() == "APPROVED"
                ]
                if not scanner.empty
                else pd.DataFrame()
            )
            if not approved.empty:
                top = approved.iloc[0]
                st.success(
                    f"TOP TRADE-GRADE OPPORTUNITY: {top['SYMBOL']} — "
                    f"{top['SIGNAL']} — {top['TRADE QUALITY']} · "
                    f"D1 {top['D1 LOCK']} · {top['D1 SENTIMENT']}"
                )
            else:
                st.warning(
                    "No trade-grade pair is currently approved. "
                    "All watched Deriv pairs were evaluated; WAIT/BLOCKED means "
                    "one or more V13 quality, D1, MTF, risk, session, news or "
                    "engine-consensus gates did not pass. The bot does not invent "
                    "a signal merely to produce one."
                )
        elif not scanner.empty:
            top = scanner.iloc[0]
            if str(top["SIGNAL"]) == "NO DATA":
                st.warning(
                    f"TOP OPPORTUNITY: {top['SYMBOL']} — NO VERIFIED DATA. "
        ...
        st.caption(
            "DERIV mode: every watched pair is independently streamed and passed "
            "through the complete V13 analysis/gating stack. Each pair has its own "
            "D1 lock and forming-D1 sentiment. A WAIT/BLOCKED result is a genuine "
            "quality decision, not missing scanner rotation."
        )
    ...
    with tabs[4]:
        st.subheader("5 · V14 Backtest / Forward Test")
    ...
    with tabs[5]:
        ...
        status = pd.DataFrame(
            [
                ...
                ["Trade Monitor", "READY"],
                ["V14 Backtest", "READY"],
                ["V14 Forward Test", "READY"],
                ["Monte Carlo", "READY"],
                ["V14 Threshold Optimizer", "READY"],
                ["Journal", "READY"],
...
def _v13_completed_daily_frame(df):
    ...
    if x.empty:
        return x
    # MarketDataEngine.normalize() intentionally resets the DataFrame index to
    # RangeIndex, so timezone operations must use the authoritative `time` column.
    times = pd.DatetimeIndex(pd.to_datetime(x["time"], errors="coerce", utc=True))
    x = x.loc[times.notna()].copy()
...
def _v13_forming_daily_frame(df):
    ...
    if x.empty:
        return x
    # MarketDataEngine.normalize() resets the index to RangeIndex; use `time`
    # for UTC/weekend handling so forming-D1 analysis never depends on index tz.
    times = pd.DatetimeIndex(pd.to_datetime(x["time"], errors="coerce", utc=True))
    x = x.loc[times.notna()].copy()
    x["time"] = times[times.notna()]
    x = x.sort_values("time")
    return x[x["time"].dt.dayofweek < 5].copy()
...
def _v14_render_architecture_testing(df, cfg, symbol, timeframe):
    ...
    st.caption(
        "Tests the actual V14 decision path chronologically. The legacy backtest "
        "and walk-forward engines have been removed; V14 is now the authoritative tester."
    )
    ...
    with t1:
        if st.button("▶ Run V14 Backtest", use_container_width=True, disabled=df.empty):
            with st.spinner("Running V14 chronological backtest…"):
                trades, metrics = V14BacktestEngine.run(df, cfg, symbol, timeframe)
    ...
    with t2:
        if st.button("↔ Run V14 Forward Test", use_container_width=True, disabled=df.empty):
            with st.spinner("Running V14 chronological forward test…"):
                result = V14ForwardTestingEngine.run(df, cfg, symbol, timeframe)
    ...
    # Existing Monte Carlo and optimizer features remain available, but they
    # now consume the authoritative V14 testing results rather than the removed
    # legacy backtest/walk-forward engines.
...
def dashboard():
    ...
    V14 testing is the authoritative backtest/forward-test path; legacy testing controls
    are not exposed.
    """
    st.set_page_config(
    ...
    with st.sidebar:
        ...
        if data_source == "TWELVE DATA":
            ...
            cfg.twelve_data_outputsize = st.number_input(
                "Historical candles", 200, 5000, 500, 100
            )
            if st.button("🔄 Refresh Twelve Data", use_container_width=True):
                try:
                    live_df, meta = _fetch_twelve_data(symbol, timeframe, cfg)
            ...
            st.session_state.mt5_path = mt5_path
            cfg.mt5_outputsize = st.number_input(
                "MT5 historical candles", 200, 5000, 500, 100
            )
            if st.button("🟢 Connect / Refresh MT5", use_container_width=True):
                try:
                    MT5LiveDataEngine.connect(symbol, path=mt5_path or None)
                    live_df = MT5LiveDataEngine.bars(symbol, timeframe, cfg.mt5_outputsize)
                    tick = MT5LiveDataEngine.tick(symbol)
                    meta = {
                        "source": "MT5",
                    ...
                    live_df, meta = store_market_data(
                        symbol, timeframe, live_df, meta, "MT5"
                    )
                    st.session_state.mt5_connected = True
                    st.session_state.live_status = LiveConnectionManager.status(
                        "MT5",
            ...
            cfg.deriv_stream_outputsize = st.number_input(
                "Deriv historical candles", 200, 5000, 500, 100
            )
    ...
    st.caption(
        "New 18-panel dashboard · specialist engines → Central Assessment → "
    ...
    # Immutable daily context remains a preserved V13 feature.
    daily_report = None
    try:
        daily_report = get_locked_daily_signal(symbol, cfg)
        if daily_report:
            analysis = refresh_trade_quality_with_daily_signal(
                analysis, daily_report
            )
    except Exception as ex:
        st.warning(f"Daily signal context unavailable: {ex}")
    ...
    with panels[3]:
        ...
        st.json({"trend": trend, "structure": structure, "price_action": analysis.get("price_action", {})})

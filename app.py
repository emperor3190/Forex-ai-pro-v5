...
Live signal logic uses completed candles for stable technical calculations and the current live quote as a separate decision-time input.
...
class Config:
    ...
    live_price_max_age_seconds: int = 15
...
class LivePriceContextEngine:
    """Live-price context layer.

    Stable technical calculations use only completed candles. The current Deriv
    quote is then evaluated separately against the completed-candle state so the
    final forecast can react to the actual price available at decision time.
    No forming candle is treated as a completed historical observation.
    """
    @staticmethod
    def analyze(df, live_price=None, live_timestamp=None):
        x = MarketDataEngine.normalize(df)
        if x.empty or live_price is None:
            return {"status":"UNAVAILABLE", "direction":"NEUTRAL", "score":50.0,
                    "live_price":None, "reference_close":float(x.close.iloc[-1]) if not x.empty else None}
        try:
            px = float(live_price)
            if not math.isfinite(px) or px <= 0:
                raise ValueError("invalid live price")
        except Exception:
            return {"status":"INVALID", "direction":"NEUTRAL", "score":50.0,
                    "live_price":None, "reference_close":float(x.close.iloc[-1])}
        ref = float(x.close.iloc[-1])
        a = float(atr(x, 14).iloc[-1]) if len(x) >= 20 else max(abs(ref)*1e-4, 1e-12)
        delta = px - ref
        atr_units = delta / max(a, 1e-12)
        # A modest bounded live-price impulse. It is evidence, not a standalone trigger.
        score = float(np.clip(50.0 + np.tanh(atr_units * 1.25) * 25.0, 0, 100))
        direction = "BULLISH" if score >= 58 else "BEARISH" if score <= 42 else "NEUTRAL"
        age = None
        if live_timestamp is not None:
            try:
                ts = pd.Timestamp(live_timestamp)
                if ts.tzinfo is None: ts = ts.tz_localize("UTC")
                age = max(0.0, (pd.Timestamp.now(tz="UTC") - ts.tz_convert("UTC")).total_seconds())
            except Exception:
                age = None
        fresh = age is None or age <= 30.0
        return {"status":"OK" if fresh else "STALE", "direction":direction,
                "score":score, "live_price":px, "reference_close":ref,
                "delta":delta, "delta_pct":delta/max(abs(ref),1e-12)*100,
                "atr_units":float(atr_units), "age_seconds":age, "fresh":fresh}


def _v13_live_price_from_df(df):
    """Read the live quote attached by the authenticated live adapter."""
    try:
        px = df.attrs.get("live_price")
        ts = df.attrs.get("live_price_timestamp")
        if px is None:
            return None, ts
        px = float(px)
        return (px if math.isfinite(px) and px > 0 else None), ts
    except Exception:
        return None, None


def _v13_completed_candles(df, live_timestamp=None):
    """Return candles known to be completed at the live decision timestamp.

    If the latest candle belongs to the currently forming interval, it is removed
    from the stable technical-analysis dataframe. Historical/backtest data without
    a live timestamp is left untouched.
    """
    x = MarketDataEngine.normalize(df)
    if x.empty or live_timestamp is None:
        return x
    try:
        ts = pd.Timestamp(live_timestamp)
        if ts.tzinfo is None: ts = ts.tz_localize("UTC")
        ts = ts.tz_convert("UTC")
        tf = str(df.attrs.get("timeframe", "")).upper()
        seconds_map = {"M1":60,"M5":300,"M15":900,"M30":1800,"H1":3600,"H4":14400,"D1":86400}
        seconds = seconds_map.get(tf)
        if seconds:
            bucket = pd.Timestamp((int(ts.timestamp()) // seconds) * seconds, unit="s", tz="UTC")
            if pd.Timestamp(x.time.iloc[-1]) >= bucket:
                x = x.iloc[:-1].copy()
...
class AIProbabilityEngine:
    ...
    @staticmethod
    def _features(df, decision_price=None):
        x=MarketDataEngine.normalize(df).copy()
        ...
        # The optional spread field is not required. Historical training rows use
        # their completed close as the decision price; live inference can replace
        # only the latest decision price with the current Deriv quote.
        if decision_price is not None and len(x):
            try:
                px=float(decision_price)
                if math.isfinite(px) and px>0:
                    prev_close=float(x.close.iloc[-2]) if len(x)>1 else float(x.close.iloc[-1])
                    x.loc[x.index[-1],"ret1"] = px/max(prev_close,1e-12)-1
                    ref3=float(x.close.iloc[-4]) if len(x)>=4 else prev_close
                    x.loc[x.index[-1],"ret3"] = px/max(ref3,1e-12)-1
                    ref10=float(x.close.iloc[-11]) if len(x)>=11 else prev_close
                    x.loc[x.index[-1],"ret10"] = px/max(ref10,1e-12)-1
                    e20=float(ema(x.close,20).iloc[-1]); e50=float(ema(x.close,50).iloc[-1])
                    x.loc[x.index[-1],"ema_gap"] = px/max(e50,1e-12)-1
                else:
                    decision_price=None
            except Exception:
                decision_price=None
    ...
    @staticmethod
    def predict(df, horizon=1, live_price=None):
        # Training uses completed-candle closes. Only the final inference row may
        # use the actual current live quote available at decision time.
        x=AIProbabilityEngine._features(df, decision_price=live_price)
        if len(x)<160:
...
class DirectProbabilityEngine:
    ...
    @staticmethod
    def predict(df, live_price=None):
        x = MarketDataEngine.normalize(df)
        if len(x) < 60:
        ...
        e20 = float(ema(x.close,20).iloc[-1]); e50 = float(ema(x.close,50).iloc[-1])
        decision_price = float(live_price) if live_price is not None and math.isfinite(float(live_price)) and float(live_price)>0 else float(x.close.iloc[-1])
        ref10 = float(x.close.iloc[-11]) if len(x) >= 11 else float(x.close.iloc[0])
        ret = decision_price / max(ref10,1e-12) - 1
        z = ((r - 50) / 20) + ((decision_price / max(e20,1e-12) - 1) * 60) + ret * 100
...
class EngineInformationBus:
    ...
    SPECS = [
        ...
        ("Current Live Price", "live_price_context", "directional", 0.90),
    ]
...
class TradeQualityEngine:
    ...
    def evaluate(a, ensemble, no_trade, data_quality, daily_signal=None):
        ...
        context_scores = [
            ...
            bus["Current Live Price"]["strength"],
        ]
...
def store_market_data(symbol: str, timeframe: str, df: pd.DataFrame, meta=None, source=None):
    ...
    # Preserve the live quote/timestamp alongside the candle frame. This metadata
    # is never used as a completed candle; it is only the current decision price.
    if m.get("live_price") is not None:
        try:
            x.attrs["live_price"] = float(m.get("live_price"))
            x.attrs["live_price_timestamp"] = m.get("live_price_timestamp")
            x.attrs["timeframe"] = str(timeframe).upper()
        except Exception:
            pass
...
def analyze_market(df, symbol, cfg, timeframe=None, *, deep_mtf=True):
    """Run V13 analysis using completed candles plus the current live quote.

    In live mode the latest forming candle is excluded from stable technical
    calculations. The current Deriv quote is carried separately into the live
    price context and the probability engines. This prevents a partial candle
    from being mistaken for a completed historical observation while still
    allowing the forecast to react to the price that exists NOW.
    """
    live_price, live_ts = _v13_live_price_from_df(df)
    source = str((st.session_state.get("data_meta", {}) or {}).get("source", "")).upper()
    is_live = source in {"DERIV REAL FOREX", "MT5", "TWELVE DATA"} and live_price is not None
    signal_df = _v13_completed_candles(df, live_ts if is_live else None)

    # HARDENED CANDLE-INTEGRITY GATE:
    # Never fall back to the raw dataframe in live mode when the completed-candle
    # frame is too short. The raw dataframe may contain the currently forming
    # candle, which would violate the completed-candle + current-live-price
    # contract and could contaminate stable indicators/probability features.
    # Fail closed instead: wait for enough genuinely completed observations.
    if is_live and len(signal_df) < 60:
        dq_insufficient = DataIntegrityEngine.assess(
            signal_df,
            timeframe or st.session_state.get("data_timeframe", "M5") or "M5",
            cfg.data_max_age_seconds,
        )
        return build_no_data_analysis(
            symbol,
            cfg,
            dq_insufficient,
            "INSUFFICIENT COMPLETED CANDLES — NO LIVE SIGNAL UNTIL THE REQUIRED "
            "HISTORICAL COMPLETED-CANDLE WINDOW IS AVAILABLE",
        )

    # Stable technical engines all read the same completed-candle decision frame.
    t = TrendEngine.analyze(signal_df)
    m = MomentumEngine.analyze(signal_df)
    v = VolatilityEngine.analyze(signal_df)
    s = StructureEngine.analyze(signal_df)
    pa = PriceActionEngine.analyze(signal_df)
    sr = SupportResistanceEngine.analyze(signal_df)
    bo = BreakoutEngine.analyze(signal_df)
    li = LiquidityEngine.analyze(signal_df)
    re = RegimeEngine.classify(t, v, s, bo)
    se = SessionEngine.analyze(live=True)
    cs = CurrencyStrengthEngine.analyze(signal_df, symbol)
    mtf = (MultiTimeframeEngine.analyze(df, symbol=symbol, cfg=cfg) if deep_mtf else MultiTimeframeEngine.analyze(signal_df))
    volume_flow = VolumeFlowEngine.analyze(signal_df)
    candle_timing = CandleTimingEngine.analyze(df, timeframe or st.session_state.get("data_timeframe", "M5") or "M5")
    market_region = MarketRegionEngine.analyze(signal_df)
    live_context = LivePriceContextEngine.analyze(signal_df, live_price, live_ts)
    direct_probability = DirectProbabilityEngine.predict(signal_df, live_price=live_price)
    eco = EconomicEngine.analyze(load_events(), symbol)
    cot = COTEngine.analyze(load_cot(), symbol)
    history = {symbol: signal_df}
    corr = CorrelationEngine.analyze(history, symbol)

    c = ConfluenceEngine.score(t, m, v, s, pa, sr, bo, li, re, se, mtf, eco, cot)
    # Current live price is an explicit, bounded component of the central confluence.
    if live_context.get("status") == "OK" and live_context.get("direction") in {"BULLISH","BEARISH"}:
        lc_dir = live_context["direction"]
        c["live_price_context"] = {"direction":lc_dir,"score":live_context["score"],"live_price":live_price}
        if c.get("direction") == lc_dir:
            c["score"] = float(np.clip(c.get("score",0.0) + min(5.0, abs(live_context["score"]-50.0)*0.10), 0, 100))
        elif c.get("direction") in {"BULLISH","BEARISH"}:
            c["score"] = float(np.clip(c.get("score",0.0) - min(4.0, abs(live_context["score"]-50.0)*0.08), 0, 100))
    dq = DataIntegrityEngine.assess(signal_df, timeframe or st.session_state.get("data_timeframe", "M5") or "M5", cfg.data_max_age_seconds)
    adv_m = MomentumDirectionEngine.analyze(signal_df)
    ai = AIProbabilityEngine.predict(signal_df, live_price=live_price)
    ...
    advisory = {"trend":t,"momentum":m,"volatility":v,"structure":s,"price_action":pa,"sr":sr,
                ...
                "advanced_momentum":adv_m,"ai":ai,"volume_flow":volume_flow,"candle_timing":candle_timing,
                "market_region":market_region,"direct_probability":direct_probability,
                "live_price_context":live_context,
                "market_tracker":{"direction":c.get("direction","WAIT"),"score":c.get("score",0),"status":"CURRENT PAIR"},
                "signal_timing":SignalTimingEngine.assess(pd.Timestamp.now(tz="UTC"), cfg.signal_max_age_seconds)}
    ...
    return {
        ...
        "breakout":bo,"liquidity":li,"regime":re,"session":se,"currency_strength":cs,
        "volume_flow":volume_flow,"candle_timing":candle_timing,"market_region":market_region,
        "direct_probability":direct_probability,"live_price_context":live_context,
        "market_tracker":advisory["market_tracker"],"signal_timing":advisory["signal_timing"],"mtf":mtf,
        "economic":eco,"cot":cot,"correlation":corr,"confluence":c,"risk":risk,
        "advanced_momentum":adv_m,"ai":ai,"ensemble":ensemble,"no_trade":no_trade,
        "trade_quality":quality,"data_quality":dq,
        "live_price":live_price,"live_price_timestamp":live_ts,
        "analysis_candle_basis":"COMPLETED CANDLES + CURRENT LIVE PRICE" if is_live else "HISTORICAL CANDLE CLOSES",
        "completed_candle_count":len(signal_df),
        "t":t,"m":m,"v":v,"s":s,"pa":pa,"bo":bo,"li":li,"re":re,"se":se,"cs":cs,"eco":eco,
        "corr":corr,"c":c,"vf":volume_flow,"ct":candle_timing,"mr":market_region,"dp":direct_probability,
    }
...
def get_deriv_real_forex_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    meta = {
        ...
        "live_price":status.get("last_quote"),
        "live_price_timestamp":status.get("last_tick"),
        "fetched_at":datetime.now(timezone.utc).isoformat(),
    }
    out = MarketDataEngine.normalize(df)
    out.attrs["live_price"] = meta.get("live_price")
    out.attrs["live_price_timestamp"] = meta.get("live_price_timestamp")
...
def dashboard():
    ...
    with tabs[6]:
        ...
        st.markdown("### Current Live Price / Forecast Input")
        st.json({"analysis_basis": a.get("analysis_candle_basis"), "live_price": a.get("live_price"),
                 "live_price_timestamp": a.get("live_price_timestamp"),
                 "live_price_context": a.get("live_price_context", {}),
                 "completed_candle_count": a.get("completed_candle_count", 0)})

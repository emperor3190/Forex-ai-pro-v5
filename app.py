...
def _v14_render_engine_ledger(analysis):
    packages = analysis.get("engine_evidence_packages", {}) if isinstance(analysis, dict) else {}
    rows = []
    for name, pkg in packages.items():
        ts = pkg.get("timestamps", {}) if isinstance(pkg, dict) else {}
        q = pkg.get("quality", {}) if isinstance(pkg, dict) else {}
        ...
        rows.append({
            ...
            "Quality": q.get("score", "—"),
            "Freshness": ts.get("freshness_status", "—"),
            "Data Age (s)": ts.get("data_age_seconds", "—"),
            "Output Timestamp": pkg.get("output_timestamp", ts.get("output_timestamp", "—")),
        })
    if rows:
        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    else:
...
def _v14_render_legacy_research_controls(df, cfg):
    """Expose the retained V13 research/backtest tools without restoring the V13 dashboard."""
    st.markdown("### Retained research tools")
    b1, b2, b3, b4 = st.columns(4)
    with b1:
        if st.button("▶ Run Backtest", use_container_width=True, disabled=df.empty):
            trades, metrics = BacktestEngine.run(df, cfg, binary=False)
            st.session_state.backtest = trades
            st.session_state.bt_metrics = metrics
    with b2:
        if st.button("↔ Run Walk-Forward", use_container_width=True, disabled=df.empty):
    ...
    with b3:
        if st.button(
            "🎲 Run Monte Carlo",
            use_container_width=True,
            disabled=st.session_state.backtest.empty,
        ):
            st.session_state.mc = MonteCarloEngine.run(st.session_state.backtest)
    with b4:
        if st.button("⚙ Run Optimizer", use_container_width=True, disabled=df.empty):
            st.session_state.optimizer = OptimizerEngine.run(df, cfg)
    ...
    if not st.session_state.backtest.empty:
        ...
        st.dataframe(st.session_state.backtest.tail(100), use_container_width=True, hide_index=True)
    if st.session_state.wf:
        st.json(st.session_state.wf)
    if st.session_state.mc:
        st.json(st.session_state.mc)
    if not st.session_state.optimizer.empty:
...
def dashboard():
    ...
    This is the primary dashboard. The former V13 dashboard remains available as
    legacy_v13_dashboard() so its existing research controls/UI code is preserved,
    but the application entry point now uses the newly designed V14 18-panel layout.
    """
    st.set_page_config(
    ...
    with panels[13]:
        ...
        p4.metric("Backtest Trades", len(st.session_state.backtest))
        if st.session_state.bt_metrics:
            st.json(st.session_state.bt_metrics)
    ...
    # Preserved research/backtesting functionality under the new dashboard.
    st.divider()
    with st.expander("🧪 Retained Backtest / Walk-Forward / Monte Carlo / Optimizer Tools", expanded=False):
        _v14_render_legacy_research_controls(df, cfg)

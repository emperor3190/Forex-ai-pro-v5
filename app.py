...
class MultiTimeframeEngine:
    ...
    def analyze(df, symbol=None, cfg=None):
        ...
        live_mode = bool(
            cfg is not None
            and str(getattr(cfg, "data_source", "DEMO")).upper() in {"TWELVE DATA", "MT5"}
            and symbol
        )
...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    outputsize = int(getattr(cfg, "mt5_outputsize", 500)) if requested_source == "MT5" else int(getattr(cfg, "twelve_data_outputsize", 500))
    ...
    if not force and key in cache:
        ...
        ttl = max(1, int(getattr(cfg, "live_refresh_seconds", 15))) if requested_source == "MT5" else float("inf")
    ...
    if requested_source == "TWELVE DATA":
        ...
        try:
            ...
            # MT5-FIRST SAFETY RULE: an MT5 failure must never silently switch
            # the analysis to Twelve Data. Twelve Data is an explicit secondary
            # source and is used only when the dashboard source is set to it.
            # This prevents a 429 Twelve Data response from interrupting or
            # changing the authoritative MT5 analysis path.
            raise RuntimeError(f"MT5 live data unavailable for {symbol}/{timeframe}: {mt5_error}")
    ...
    resolved_size = int(getattr(cfg, "mt5_outputsize", 500)) if actual_source == "MT5" else int(getattr(cfg, "twelve_data_outputsize", 500))
...
def dashboard():
    ...
    with st.sidebar:
        ...
        # MT5 is deliberately first and is the default authoritative live source.
        # Twelve Data and DEMO remain available as explicit, manual alternatives.
        source_options = ["MT5", "TWELVE DATA", "DEMO"]
        ...
        cfg.allow_live_source_failover = st.checkbox(
            ...
            value=False,
            disabled=(data_source == "MT5"),
            help=(
                "Disabled in MT5 mode: MT5 is authoritative and no Twelve Data fallback is permitted. "
                "Twelve Data is used only when explicitly selected."
            ),
        )

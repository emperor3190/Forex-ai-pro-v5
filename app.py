...
adds strict dashboard-pair/timeframe/source synchronization so scanners and analysis cannot silently
use candles belonging to a different currency pair or provider. Twelve Data is the default live
source for real FX candles; synthetic data is never substituted for a missing live pair.
...
class TwelveDataLiveEngine:
    ...
    def fetch(api_key, symbol, timeframe="M5", outputsize=500, timeout=15):
        ...
        params={"symbol":provider_symbol,"interval":TwelveDataLiveEngine.TF_MAP.get(timeframe,timeframe),
        ...
        return df, {"source":"TWELVE DATA","rows":len(df),"fetched_at":datetime.now(timezone.utc).isoformat(),
                    "requested_symbol":provider_symbol,"canonical_symbol":canonical_symbol(symbol),
...
def twelve_data_symbol(symbol: str) -> str:
    """Return Twelve Data's standard FX/instrument notation without changing the dashboard key."""
    s = canonical_symbol(symbol)
    # Twelve Data commonly uses slash notation for FX pairs. Keep the dashboard
    # identity canonical (AUDUSD) while requesting AUD/USD from the provider.
...
def data_matches_selection(symbol: str, timeframe: str, df=None, source: str | None = None) -> bool:
    """True only when stored candles belong to the exact pair/timeframe/source selection."""
    if df is None or df.empty:
        return False
...
def store_market_data(symbol: str, timeframe: str, df: pd.DataFrame, meta=None, source=None):
    """Atomically store candles and the identity of the candles."""
    ...
    m.update({
        ...
        "source": source or m.get("source", "UNKNOWN"),
    })
...
def clear_market_data_identity():
    """Invalidate the previous pair identity so stale candles cannot be analyzed as another pair."""
...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    """Return live data for exactly one pair. Never substitutes synthetic data in live modes."""
    symbol = canonical_symbol(symbol)
    timeframe = str(timeframe).upper()
    source = str(cfg.data_source).upper()
    cache = st.session_state.setdefault("live_pair_cache", {})
    key = (source, symbol, timeframe, int(getattr(cfg, "twelve_data_outputsize", 500)))

    if not force and key in cache:
        cached = cache[key]
        cached_meta = cached.get("meta", {}) or {}
        if (cached.get("df") is not None and not cached["df"].empty
                and canonical_symbol(cached_meta.get("symbol", "")) == symbol
                and str(cached_meta.get("timeframe", "")).upper() == timeframe
                and str(cached_meta.get("source", "")).upper() == source):
            return cached["df"], cached_meta

    if source == "TWELVE DATA":
        df, meta = TwelveDataLiveEngine.fetch(
            cfg.twelve_data_api_key, symbol, timeframe, cfg.twelve_data_outputsize
        )
        meta = dict(meta or {})
        meta.update({"symbol": symbol, "timeframe": timeframe, "source": "TWELVE DATA"})
    elif source == "MT5":
        ...
        meta = {"symbol": symbol, "timeframe": timeframe, "source": "MT5", "read_only": True, "execution_enabled": False, "tick": tick}
    else:
        # DEMO is the only mode permitted to use the built-in synthetic generator.
        df = MarketDataEngine.synthetic(symbol, cfg.demo_rows, seed=7 + sum(map(ord, symbol)))
    ...
    cache[key] = {"df": df.copy(), "meta": meta}
    return df, meta


def get_selected_market_data(symbol: str, timeframe: str, cfg: Config):
    """Resolve the dashboard selection without ever reusing another pair's candles."""
    symbol = canonical_symbol(symbol)
    timeframe = str(timeframe).upper()
    source = str(cfg.data_source).upper()

    # Existing data is valid only when its recorded identity exactly matches the UI.
    if data_matches_selection(symbol, timeframe, st.session_state.get("data"), source):
        return st.session_state.data, st.session_state.get("data_meta", {})

    if source in ("TWELVE DATA", "MT5"):
        df, meta = get_live_pair_data(symbol, timeframe, cfg)
        return store_market_data(symbol, timeframe, df, meta, source)

    # Demo mode is explicit and therefore safe to regenerate for the newly selected symbol.
    df, meta = get_live_pair_data(symbol, timeframe, cfg)
    return store_market_data(symbol, timeframe, df, meta, "DEMO")
...
def init_state():
    defaults = {
        ...
        "active_data_source": "TWELVE DATA",
        "td_key": "",
        "live_status": {"source":"TWELVE DATA","connected":False,"read_only":True,"execution_enabled":False},
...
def analyze_market(df, symbol, cfg):
    ...
    dq = DataIntegrityEngine.assess(df, st.session_state.get("data_timeframe", "M5"), cfg.data_max_age_seconds)
...
def dashboard():
    ...
    with st.sidebar:
        ...
        symbol = st.text_input("Primary symbol", "EURUSD", key="primary_symbol").upper().strip()
        market = st.selectbox("Market", ["FOREX", "BINARY OPTIONS"], key="market_mode")
        ...
        data_source = st.selectbox(
            "Data source", ["TWELVE DATA", "MT5", "DEMO"],
            index=0, key="data_source",
            help="TWELVE DATA reads real FX/market candles. DEMO is the only synthetic source. MT5 requires a local MetaTrader5 installation."
        )
        cfg.data_source = data_source
        previous_source = st.session_state.get("active_data_source")
        if previous_source != data_source:
            # Never carry candles across providers. Pair/timeframe alone is not enough.
            st.session_state.active_data_source = data_source
            st.session_state.live_status = {
                "source": data_source, "connected": False, "read_only": True,
                "execution_enabled": False, "error": None
            }
            clear_market_data_identity()
        if data_source == "TWELVE DATA":
            cfg.twelve_data_api_key = st.text_input(
                "Twelve Data API key", type="password",
                value=st.session_state.get("td_key", ""), key="td_api_key"
            )
            ...
            if st.button("🔄 Refresh Twelve Data", use_container_width=True):
                try:
                    ...
                    st.success(f"Twelve Data: {len(live_df):,} real candles loaded for {canonical_symbol(symbol)} · {timeframe} (provider symbol: {twelve_data_symbol(symbol)}).")
            ...
            if st.button("🟢 Connect / Refresh MT5 Live Data", use_container_width=True):
                try:
                    ...
                    if mt5 is None:
                        st.warning("MT5 is unavailable in this environment. Select TWELVE DATA and enter your Twelve Data API key to read real currency-pair candles. No synthetic data will be substituted for MT5.")
        else:
    ...
    if up:
        try:
            ...
            st.success(f"Loaded {len(st.session_state.data):,} candles for {symbol} · {timeframe}.")
        except Exception as e:
            st.error(f"CSV error: {e}")
    else:
        st.info(
            f"{cfg.data_source} mode active. Primary pair/timeframe: {symbol} · {timeframe}. "
    ...
    try:
        ...
        # Never fall back to another pair's candles when a live pair refresh fails.
        clear_market_data_identity()
        st.session_state.live_status = {"source": cfg.data_source, "connected": False, "read_only": True, "execution_enabled": False, "error": str(ex)}
        st.error(f"Market data error for {canonical_symbol(symbol)}/{timeframe} from {cfg.data_source}: {ex}")
    ...
    validation = MarketDataEngine.validate(df) if not df.empty else {"rows":0,"data_ok":False,"duplicates_removed":0,"missing_ohlc":0,"large_gaps":0,"timezone":"UTC"}
    data_quality = DataIntegrityEngine.assess(df, timeframe, cfg.data_max_age_seconds)
    a = analyze_market(df, symbol, cfg) if not df.empty else {
        "c": {"score": 0.0, "direction": "WAIT", "components": {}, "quality": 0.0},
        "t": {"label": "NO DATA", "direction": "SIDEWAYS"}, "m": {"direction": "NEUTRAL"}, "v": {"regime": "EXTREME"},
        "s": {"direction": "TRANSITION"}, "pa": {"direction": "NEUTRAL"}, "sr": {}, "bo": {}, "li": {}, "re": "NO-TRADE",
        "se": SessionEngine.analyze(), "cs": {}, "mtf": {"alignment": 0, "strength": "WEAK", "states": {}},
        "eco": EconomicEngine.analyze(load_events(), symbol), "cot": COTEngine.analyze(load_cot(), symbol),
        "corr": {"risk": "LOW"}, "risk": {"approved": False, "status": "VETO", "reasons": ["NO MARKET DATA"], "risk_pct": cfg.risk_per_trade*100},
        "advanced_momentum": {"direction": "NEUTRAL"}, "ai": {"up_probability":50.0,"down_probability":50.0,"confidence":0.0},
        "ensemble": {"score":0.0,"direction":"WAIT","approved":False,"reasons":["NO MARKET DATA"]},
        "no_trade": {"trade_allowed":False,"reasons":["NO MARKET DATA"],"status":"NO TRADE"},
        "trade_quality": {"grade":"D","score":0.0,"decision":"NO TRADE","checks":{}}, "data_quality": data_quality,
    }
    ...
    loaded_source = str((st.session_state.get("data_meta", {}) or {}).get("source", "")).upper()
    sync_ok = bool(
        not df.empty
        and loaded_symbol == selected_symbol
        and str(st.session_state.get("data_timeframe", "")).upper() == str(timeframe).upper()
        and loaded_source == str(cfg.data_source).upper()
    )
    if sync_ok:
        st.success(f"🔗 DATA SYNC LOCKED · Dashboard {selected_symbol} = Loaded {loaded_symbol} · {timeframe} · Source {loaded_source}")
    else:
        st.error(f"⛔ DATA SYNC BLOCKED · Dashboard {selected_symbol}/{timeframe} does not match loaded data {loaded_symbol or 'NONE'}/{st.session_state.get('data_timeframe') or 'NONE'} · Source {loaded_source or 'NONE'}")
    ...
    st.caption(
        f"Pair: {symbol} · Loaded data pair: {st.session_state.get('data_symbol') or 'NONE'} · "
        f"Timeframe: {timeframe} · Loaded source: {loaded_source or 'NONE'} · Data: {'OK' if validation['data_ok'] else 'INSUFFICIENT'} · UTC · "
    ...
    with tabs[0]:
        ...
        st.caption(
            f"Scanner source: {cfg.data_source} · Every live row is fetched for its own symbol/timeframe; "
            "synthetic candles are used only when DEMO is explicitly selected."
        )
        analyses = {}
        scanner_rows = []
        for sym in watch:

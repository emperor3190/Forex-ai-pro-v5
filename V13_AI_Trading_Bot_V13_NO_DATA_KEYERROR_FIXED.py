...
def data_matches_selection(symbol: str, timeframe: str, df=None, source: str | None = None) -> bool:
    """True only when stored candles belong to the exact dashboard selection and source."""
    if df is None or df.empty:
        return False
...
def get_selected_market_data(symbol: str, timeframe: str, cfg: Config):
    ...
    # Never reuse candles from another data source, even when pair/timeframe match.
...
def dashboard():
    ...
    if analysis_error:
        st.error(f"Analysis engine error for {symbol}/{timeframe}: {analysis_error}")
    ...
    with tabs[1]:
        ...
        if isinstance(cs_matrix, dict) and cs_matrix:
            ...
            st.info(
                f"Currency strength unavailable for {symbol} · {timeframe}. "
    ...
    with tabs[2]:
        ...
        if df.empty:
            st.warning(
                f"NO TRADE DATA · {symbol} · {timeframe}. Trade calculations are disabled until valid candles are loaded."
            )
            direction = "WAIT"
        else:

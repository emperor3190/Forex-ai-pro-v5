...
"""
V14 AI Forex Trading Bot — Accuracy, Robustness & Unified Real-Forex Data V14
...
synchronization so scanners and analysis cannot silently use candles belonging to a different currency pair. Deriv public market-data bootstrap is deliberately separated from the full historical-test acquisition so the dashboard can render without waiting for 12,000 candles. The Deriv real-Forex source uses the
...
class CentralAssessmentDataQualityEngine:
    ...
    This layer NEVER creates BUY/SELL/NO-TRADE. It only determines whether the
    evidence package is trustworthy enough to proceed to Signal Intelligence.
    """
    @staticmethod
    def assess(
        symbol: str,
        ...
        for name, pkg in (evidence_packages or {}).items():
            ...
            if score is not None:
                try:
                    ...
                    pass
            q_status = str(q.get("status", "")).upper()
            if q_status in {"FAIL", "FAILED", "INVALID", "STALE"}:
                invalid.append(name)
            elif q_status in {"UNAVAILABLE", "INSUFFICIENT"}:
                unavailable.append(name)
            ev = pkg.get("evidence", {}) if isinstance(pkg, dict) else {}
            if ev.get("conflict_status") not in (None, "", "NONE"):
                conflicts.append({"engine": name, "status": ev.get("conflict_status")})
        ...
        reasons = []
        rows = int(market_quality.get("rows", 0) or 0)
        validation_status = str(market_quality.get("validation_status", "UNKNOWN")).upper()
        validation_allowed = bool(market_quality.get("validation_signal_allowed", False))
        integrity_allowed = bool(market_quality.get("signal_allowed", False))
        if rows <= 0:
            reasons.append("NO_VERIFIED_MARKET_DATA")
        if validation_status != "PASS" or not validation_allowed:
            reasons.append("DATA_VALIDATION_NOT_PASSED")
        if not integrity_allowed:
            reasons.append("DATA_INTEGRITY_GATE_BLOCKED")
        if overall < 60:
            reasons.append("OVERALL_DATA_QUALITY_BELOW_THRESHOLD")
        if invalid:
            reasons.append("INVALID_OR_STALE_ENGINE_EVIDENCE")
        if conflicts:
            reasons.append("ENGINE_CONFLICTS_PRESENT")
        if not reasons and unavailable:
            reasons.append("SOME_EVIDENCE_UNAVAILABLE")
        ...
        return {
            "symbol": symbol,
            "timeframe": timeframe,
            "data_source": market_quality.get("source", "UNKNOWN"),
            ...
            "validation_score": float(market_quality.get("validation_score", 0.0) or 0.0),
            "validation_status": validation_status,
            "validation_signal_allowed": validation_allowed,
            "engine_quality_score": engine_score,
            "overall_data_quality_score": overall,
            "engine_count": len(evidence_packages or {}),
            ...
            "data_flow_status": "VALIDATED_TO_CENTRAL" if ready else "BLOCKED_BEFORE_CENTRAL_DECISION",
        }
...
class Config:
    ...
    historical_target_rows: int = 1200
    historical_max_requests: int = 4
    minimum_history_candles: int = 1200
    deriv_history_min_interval_seconds: int = 5
    deriv_history_rate_limit_cooldown_seconds: int = 120
    minimum_statistically_valid_trades: int = 30
    minimum_statistically_valid_oos_trades: int = 20
    data_source: str = "DERIV REAL FOREX"
    ...
    # Additive Deriv real-Forex market-data stream settings.
    # PUBLIC is the default because Deriv documents ticks/ticks_history as no-auth market-data endpoints.
    # Authenticated mode remains optional and is never required merely to load the dashboard.
    deriv_api_base: str = "https://api.derivws.com"
    # Provider endpoint is resolved through the Deriv market-data connection manager.
    deriv_stream_endpoint: str = "PUBLIC MARKET DATA"
    deriv_market_data_mode: str = "PUBLIC"
    deriv_stream_enabled: bool = False
    deriv_stream_outputsize: int = 1200
    deriv_stream_reconnect_seconds: int = 5
    deriv_stream_stale_seconds: int = 15
    deriv_live_start_timeout_seconds: int = 45
    deriv_history_timeout_seconds: int = 180
...
class MultiTimeframeEngine:
    ...
    def _direct_live_data(df, symbol, timeframe, cfg):
        ...
        # DERIV REAL FOREX: reuse the same public market-data stream.
        # IMPORTANT: dashboard MTF analysis must NEVER require PAT/account
        # credentials merely to read public market data.
        if requested_source == "DERIV REAL FOREX":
            market_data_mode = str(getattr(cfg, "deriv_market_data_mode", "PUBLIC") or "PUBLIC").upper()
            if market_data_mode == "AUTHENTICATED":
                app_id, token, configured_account = DerivRealForexStream.resolve_credentials(
                    getattr(cfg, "deriv_options_account_id", "")
                )
            else:
                app_id, token, configured_account = "", "", ""
            ...
            if frame is None or frame.empty:
                tick_bars = OHLCVBuilderEngine.build_from_ticks(
                    stream.ticks(), str(timeframe).upper(), symbol=canonical_symbol(symbol), source="DERIV REAL FOREX"
                )
                if not tick_bars.empty:
                    frame = tick_bars
            if frame is None or frame.empty:
                return _empty_market_data(), {"source":"DERIV REAL FOREX","direct":True,"pending":True,"stream_reused":True,"historical_request_sent":False,"stream_health":status}
            if not status.get("connected"):
                raise RuntimeError(
                    f"Deriv market-data WebSocket is not connected for {canonical_symbol(symbol)}/{str(timeframe).upper()}: {status}"
                )
            # Do not require a tick for historical/MTF bootstrap; a public
            # candles response is valid market data. If ticks exist, stale-feed
            # protection is enforced by the caller's data-integrity layer.
            return MarketDataEngine.normalize(frame), {
                "source": "DERIV REAL FOREX",
                "direct": True,
                "authenticated": bool(market_data_mode == "AUTHENTICATED"),
                "connection_mode": market_data_mode,
                "stream_reused": True,
...
class V14BacktestEngine:
    ...
    # Minimum executable historical dataset requested by the operator.
    # 1,200 candles is the execution threshold; statistical-validation warnings
    # remain active and must be respected when the sample is small.
    MIN_CANDLES = 1200
    WARMUP = 120
    MAX_HOLDING_BARS = 24
    AI_REFRESH_BARS = 5
...
class HistoricalDatasetBuilder:
    ...
    def build_twelve_data(cls, symbol: str, timeframe: str, cfg: Config,
        ...
        target = max(1200, int(target_rows or getattr(cfg, "historical_target_rows", 1200)))
        max_req = max(1, int(max_requests or getattr(cfg, "historical_max_requests", 4)))
        chunk = min(5000, max(500, int(getattr(cfg, "twelve_data_outputsize", 500))))
        key = _get_twelve_data_key(cfg)
        if not key:
...
def ensure_historical_test_dataset(df: pd.DataFrame, cfg: Config, symbol: str, timeframe: str):
    ...
    target = max(1200, int(getattr(cfg, "historical_target_rows", 1200) or 1200))
    if source == "DERIV REAL FOREX":
        ...
        try:
            cfg.deriv_stream_outputsize = max(int(getattr(cfg, "deriv_stream_outputsize", 500) or 500), target)
            built, meta = get_deriv_real_forex_data(symbol, timeframe, cfg, force=False, required_rows=target)
    ...
    if len(x) >= int(getattr(cfg, "minimum_history_candles", 500) or 500):
        return x, {"status": "READY", "rows": len(x), "expanded": False}
    if source == "TWELVE DATA":
        target = max(1200, int(getattr(cfg, "historical_target_rows", 1200)))
        built, meta = HistoricalDatasetBuilder.build_twelve_data(symbol, timeframe, cfg, target_rows=target)
        return built, {"status": "READY" if len(built) >= 1200 else "INSUFFICIENT_REAL_HISTORY",
...
def get_live_pair_data(symbol: str, timeframe: str, cfg: Config, force=False):
    ...
    if requested_source == "DERIV REAL FOREX" and not force:
        try:
            mdm=str(getattr(cfg,"deriv_market_data_mode","PUBLIC") or "PUBLIC").upper(); app_id=token=account=""
            if mdm == "AUTHENTICATED": app_id,token,account=DerivRealForexStream.resolve_credentials(getattr(cfg,"deriv_options_account_id",""))
            stream0=DerivRealForexStream.get(symbol,cfg.deriv_api_base,app_id,token,account,cfg.deriv_stream_outputsize,cfg.deriv_stream_reconnect_seconds,mdm)
            existing0=stream0.frame(timeframe)
            if existing0 is not None and not existing0.empty:
                snap0=stream0.snapshot(); return MarketDataEngine.normalize(existing0), {"symbol":symbol,"timeframe":timeframe,"source":"DERIV REAL FOREX","provider_symbol":stream0.deriv_symbol,"synthetic":False,"read_only":True,"execution_enabled":False,"streaming":bool(snap0.get("streaming")),"connection_mode":mdm,"reused_stream_cache":True,"historical_rows":int(len(existing0)),"stream_health":snap0,"fetched_at":datetime.now(timezone.utc).isoformat()}
        except Exception:
            pass
    ...
    try:
        if requested_source == "TWELVE DATA":
            ...
            # LIVE PATH ONLY. This function never calls ticks_history.
            df, meta = get_deriv_live_market_data(symbol, timeframe, cfg)
...
def analyze_market(df, symbol, cfg, timeframe=None, *, deep_mtf=True):
    ...
    # Central Assessment must see BOTH integrity and dedicated validation results.
    # This keeps provider provenance and the validated OHLCV gate in one quality package.
    dq["validation_status"] = str(validation.get("status", "FAIL")).upper()
    dq["validation_score"] = float(validation.get("score", 0.0) or 0.0)
    dq["validation_signal_allowed"] = bool(validation.get("signal_allowed", False))
    dq["signal_allowed"] = bool(dq.get("signal_allowed", False) and validation.get("signal_allowed", False))
    dq["source"] = str((st.session_state.get("data_meta", {}) or {}).get("source", cfg.data_source)).upper()
    dq["rows"] = int(len(df))
    ...
    evidence_inputs = {
        # Core technical-analysis evidence engines. None of these owns a final signal.
        "TrendEngine": t,
        ...
        "AdvancedMomentumEngine": adv_m,
        # Context / predictive evidence also enters Central Assessment after validation.
        "MTFConfirmation": mtf,
        "AIProbability": ai,
        "SessionEngine": se,
        "EconomicCalendarEventEngine": {"events": economic_events, "status": "OK" if economic_events is not None else "UNAVAILABLE"},
        "EconomicEngine": eco,
        "NewsEventFilterEngine": news_filter,
        "COTEngine": cot,
        "CandleTimingEngine": candle_timing,
        "MarketRegionEngine": market_region,
        "DirectProbabilityEngine": direct_probability,
        "MarketTrackerEngine": advisory["market_tracker"],
        "ReversalContext": {"direction": re if re in {"BULLISH","BEARISH"} else "NEUTRAL", "context": re},
    }
    evidence_packages = {}
...
def warm_deriv_multi_pair_streams(symbols, cfg):
    """Ensure every scanner pair has its own Deriv read-only market-data stream.

    Public market data is preferred and requires no PAT/OTP. Authenticated mode remains
    available only when explicitly configured. Each pair gets one reusable WebSocket.
    """
    ...
    def start_one(sym):
        market_data_mode = str(getattr(cfg, "deriv_market_data_mode", "PUBLIC") or "PUBLIC").upper()
        if market_data_mode == "AUTHENTICATED":
            app_id, token, configured_account = DerivRealForexStream.resolve_credentials(getattr(cfg, "deriv_options_account_id", ""))
        else:
            app_id, token, configured_account = "", "", ""
        stream = DerivRealForexStream.get(
            sym,
        ...
        # Warm ONLY the reusable tick connection. No historical request is armed.
        # Historical candles are acquired by the explicit testing dataset action.
...
# ============================================================
# V14 ADDITION: DERIV PUBLIC REAL-FOREX LIVE STREAM (READ-ONLY)
# ============================================================
class DerivHistoryRateGuard:
    """Process-wide gate preventing Deriv historical request storms."""
    _lock = threading.RLock()
    _last_request = {}
    _cooldown_until = {}
    @classmethod
    def reserve(cls, key, min_interval=5):
        # Deriv's WebSocket "all other calls" budget is shared; keying by symbol
        # or connection can therefore still exceed the provider-wide budget.
        key = "DERIV_GLOBAL_HISTORY"
        now = _time.time()
        with cls._lock:
            until = float(cls._cooldown_until.get(key, 0.0) or 0.0)
            if until > now:
                return False, max(1, int(math.ceil(until-now))), "RATE_LIMIT_COOLDOWN"
            last = float(cls._last_request.get(key, 0.0) or 0.0)
            wait = float(min_interval) - (now-last)
            if wait > 0:
                return False, max(1, int(math.ceil(wait))), "PACE"
            cls._last_request[key] = now
            return True, 0, "OK"
    @classmethod
    def rate_limited(cls, key, cooldown_seconds=180):
        key = "DERIV_GLOBAL_HISTORY"
        with cls._lock:
            cls._cooldown_until[key] = max(float(cls._cooldown_until.get(key,0.0) or 0.0), _time.time()+max(60,int(cooldown_seconds)))
    @classmethod
    def status(cls, key):
        key = "DERIV_GLOBAL_HISTORY"
        now=_time.time()
        with cls._lock:
            until=float(cls._cooldown_until.get(key,0.0) or 0.0)
            return {"cooldown":until>now,"cooldown_seconds":max(0,int(math.ceil(until-now))) if until>now else 0,"last_request_at":cls._last_request.get(key)}


class DerivRealForexStream:
    """Read-only Deriv real-Forex market-data stream.

    PUBLIC mode is the primary path: it requires no PAT, account discovery, or OTP.
    AUTHENTICATED mode remains available only when explicitly selected. Both modes
    use the same Deriv Forex symbols and the same Tick -> OHLCV -> Validation path.
    No synthetic candles are ever created.

    Current Deriv public endpoint:
      wss://api.derivws.com/trading/v1/options/ws/public

    A legacy public WebSocket endpoint is retained as a compatibility fallback when
    an App ID is available. The fallback is still read-only market data.
    """
    ...
    def __init__(self, symbol, api_base, app_id="", token="", account_id="", outputsize=500, reconnect_seconds=5, market_data_mode="PUBLIC"):
        ...
        self.market_data_mode = str(market_data_mode or "PUBLIC").upper()
        ...
        self.history_page_size = min(5000, max(1200, self.outputsize))
        ...
        self.history_complete = {tf: False for tf in self.TF_SECONDS}
        self.history_errors = {tf: None for tf in self.TF_SECONDS}
        self.history_queue = []
        self.history_inflight = None
        self.history_retry_count = 0
        self.history_min_interval_seconds = 5
        self.history_rate_limit_cooldown_seconds = 120
        self.history_request_key = "DERIV_GLOBAL_HISTORY"
        self.history_timer = None
        self.required_timeframe = None
        self.required_rows = None
        # Historical acquisition is an explicit job only. Live stream startup and
        # reconnects must never implicitly call ticks_history.
        self.history_acquisition_requested = False
        # A live dashboard needs a verified historical seed so the technical
        # engines have enough context immediately. This flag guarantees the
        # seed request is attempted only once per stream/process unless the
        # operator explicitly starts a new stream. Live ticks then update the
        # same OHLCV series.
        self.live_history_bootstrap_attempted = False
        self.live_history_bootstrap_at = None
        self.status = {
            "connected":False,
            "authenticated":False,
            "validated":True,
            ...
            "account_type":"demo" if self.market_data_mode == "AUTHENTICATED" else "public",
            "endpoint": None,
            "account_id":self.account_id,
            "connection_mode": self.market_data_mode,
            "last_tick":None,
            "last_quote":None,
            "error":None,
            "streaming":False,
            ...
            "read_only":True,
            "execution_enabled":False,
            "history_target_rows": self.outputsize,
            "history_page_size": self.history_page_size,
            "history_complete": dict(self.history_complete),
            "history_rows": {tf: 0 for tf in self.TF_SECONDS},
            "history_pages": dict(self.history_pages),
            "history_oldest": dict(self.history_oldest),
            "history_inflight": None,
            "history_error": None,
        }

    @classmethod
    def get(cls, symbol, api_base, app_id="", token="", account_id="", outputsize=500, reconnect_seconds=5, market_data_mode="PUBLIC"): 
        key = (canonical_symbol(symbol), str(account_id or ""), str(market_data_mode or "PUBLIC").upper())
        with cls._registry_lock:
            obj = cls._registry.get(key)
            if obj is None:
                obj = cls(key[0], api_base, app_id, token, account_id, outputsize, reconnect_seconds, market_data_mode)
                ...
                obj.market_data_mode = str(market_data_mode or "PUBLIC").upper()
            return obj

    def _send(self, payload):
        """Best-effort WebSocket send; never let a closed socket crash Streamlit."""
        try:
            if self.ws is None or not self.status.get("connected"):
                return False
            self.ws.send(json.dumps(payload))
            return True
        except Exception as exc:
            with self.lock:
                self.status["error"] = f"Deriv WebSocket send failed: {exc}"
    ...
    def _on_open(self, ws):
        with self.lock:
            self.status.update({"connected":True,"authenticated":self.market_data_mode == "AUTHENTICATED","streaming":False,"error":None,"connection_mode":self.market_data_mode})
            for tf in self.TF_SECONDS:
                self.history_pages[tf] = 0
                self.history_oldest[tf] = None
                self.history_complete[tf] = False
                self.history_errors[tf] = None
            # CRITICAL SEPARATION: reconnecting the live stream must NEVER start
            # historical acquisition. ticks_history is allowed only when an explicit
            # historical job has armed history_acquisition_requested.
            if self.history_acquisition_requested and self.required_timeframe in self.TF_SECONDS:
                self.history_queue = [self.required_timeframe]
            else:
                self.history_queue = []
            self.history_inflight = None
            self.history_retry_count = 0
            self.status["history_error"] = None
        self._send_next_history_request()
        self._send({"ticks":self.deriv_symbol,"subscribe":1,"req_id":3001})

    def _schedule_history_request(self, delay_seconds):
        delay=max(0.5,float(delay_seconds))
        with self.lock:
            if self.history_timer is not None and self.history_timer.is_alive(): return
            self.history_timer=threading.Timer(delay,self._send_next_history_request)
            self.history_timer.daemon=True; self.history_timer.start()

    def _send_next_history_request(self):
        with self.lock:
            if self.history_inflight is not None: return
            while self.history_queue and self.history_complete.get(self.history_queue[0],False): self.history_queue.pop(0)
            if not self.history_queue: return
            tf=str(self.history_queue[0]).upper(); seconds=self.TF_SECONDS[tf]; page=int(self.history_pages.get(tf,0))
            if page >= self.history_max_pages:
                self.history_complete[tf]=False; self.history_errors[tf]=f"Maximum historical pages reached before target ({self.outputsize:,} candles)."
                self.history_queue.pop(0); self.status["history_error"]=self.history_errors[tf]; schedule=True
            else:
                end="latest" if page==0 else (int(pd.Timestamp(self.history_oldest[tf]).timestamp())-1 if self.history_oldest.get(tf) is not None else "latest")
                allowed,wait,reason=DerivHistoryRateGuard.reserve(self.history_request_key,self.history_min_interval_seconds)
                if not allowed:
                    self.status["history_rate_guard"]={"blocked":True,"wait_seconds":wait,"reason":reason}; schedule=False; send=False
                else:
                    self.history_pages[tf]=page+1; self.history_inflight={"timeframe":tf,"page":page,"end":end}; self.history_retry_count=0
                    self.status["history_inflight"]=dict(self.history_inflight); self.status["history_rate_guard"]={"blocked":False,"reason":"OK"}
                    req_id=2000+list(self.TF_SECONDS).index(tf)*100+page; schedule=False; send=True
        if schedule:
            self._send_next_history_request(); return
        if not send:
            self._schedule_history_request(wait); return
        sent=self._send({"ticks_history":self.deriv_symbol,"count":self.history_page_size,"end":end,"style":"candles","granularity":seconds,"req_id":req_id,"subscribe":0})
        if not sent:
            with self.lock:
                if self.history_inflight:
                    tf=self.history_inflight["timeframe"]; self.history_pages[tf]=max(0,self.history_pages.get(tf,1)-1); self.history_inflight=None
            self._schedule_history_request(self.history_min_interval_seconds)

    def _finish_history_request(self, tf: str, success: bool, error_message: str = None):
        with self.lock:
            if error_message:
                self.history_errors[tf] = str(error_message)
                self.status["history_error"] = str(error_message)
                # Terminal historical failures must not turn a later reconnect
                # into an unsolicited ticks_history retry.
                self.history_acquisition_requested = False
                self.history_queue = []
            elif success:
                self.history_errors[tf] = None
            self.history_inflight = None
            self.status["history_inflight"] = None
            self.status["history_complete"] = dict(self.history_complete)
            self.status["history_rows"] = {k: int(len(v)) for k, v in self.frames.items()}
            self.status["history_pages"] = dict(self.history_pages)
            self.status["history_oldest"] = {k: (str(v) if v is not None else None) for k, v in self.history_oldest.items()}
        self._send_next_history_request()
    ...
    def _on_message(self, ws, raw):
        ...
        if data.get("error"):
            ...
            message = err.get("message", "Deriv WebSocket error")
            with self.lock:
                self.status["error"] = message
                active = dict(self.history_inflight) if self.history_inflight else None
            if active is not None:
                tf=active["timeframe"]
                is_rate_limit=("ratelimit" in str(err.get("code","")).replace("_","").lower() or "rate limit" in str(message).lower() or "rate_limit" in str(message).lower())
                if is_rate_limit:
                    DerivHistoryRateGuard.rate_limited(self.history_request_key,self.history_rate_limit_cooldown_seconds)
                    with self.lock:
                        self.history_pages[tf]=max(0,int(self.history_pages.get(tf,1))-1); self.history_inflight=None; self.status["history_inflight"]=None; self.history_errors[tf]=message
                        self.history_acquisition_requested = False
                        self.history_queue = []
                        self.status["history_rate_guard"]={"blocked":True,"reason":"DERIV_RATE_LIMIT",**DerivHistoryRateGuard.status(self.history_request_key)}
                    self._schedule_history_request(DerivHistoryRateGuard.status(self.history_request_key).get("cooldown_seconds",120))
                elif self.history_retry_count < 1:
                    with self.lock:
                        self.history_retry_count += 1; self.history_pages[tf]=max(0,int(self.history_pages.get(tf,1))-1); self.history_inflight=None; self.status["history_inflight"]=None
                    self._schedule_history_request(self.history_min_interval_seconds)
                else:
                    self._finish_history_request(tf,False,message)
            return
        typ = data.get("msg_type")
        if typ == "candles":
            # Current Deriv docs explicitly state echo_req is optional. We therefore
            # correlate the response to the single history request known to be in flight.
            with self.lock:
                active = dict(self.history_inflight) if self.history_inflight else None
            if active is None:
                return
            tf = str(active["timeframe"]).upper()
            page = int(active["page"])
            rows = data.get("candles", []) or []
            frame = pd.DataFrame(rows)
            if frame.empty:
                self._finish_history_request(tf, False, f"Deriv returned an empty candles page for {tf} page {page}.")
                return
            try:
                ...
                if frame.empty:
                    raise ValueError("Deriv candles page contained no valid OHLC rows.")
                built = OHLCVBuilderEngine.build(frame, tf)
                if built.empty:
                    raise ValueError("OHLCV Builder returned no valid candles for the Deriv page.")
                with self.lock:
                    ...
                    previous_oldest = self.history_oldest.get(tf)
                    self.history_oldest[tf] = oldest
                    have = len(self.frames[tf])
                    got_page = len(built)
                    target_reached = have >= int(self.required_rows or self.outputsize)
                    short_page = got_page < self.history_page_size
                    backwards_progress = previous_oldest is None or oldest is None or pd.Timestamp(oldest) < pd.Timestamp(previous_oldest)
                    complete = bool(target_reached or (short_page and backwards_progress and have > 0))
                    if not backwards_progress and not target_reached:
                        complete = False
                        self.history_errors[tf] = "Historical pagination stopped: provider page did not move backward."
                    self.history_complete[tf] = complete
                    self.status["history_rows"][tf] = int(have)
                    self.status["history_pages"][tf] = int(self.history_pages.get(tf, 0))
                    self.status["history_complete"][tf] = complete
                    self.status["history_oldest"][tf] = str(oldest) if oldest is not None else None
                if complete:
                    with self.lock:
                        if self.history_queue and self.history_queue[0] == tf:
                            self.history_queue.pop(0)
                        self.history_acquisition_requested = False
                    self._finish_history_request(tf, True)
                else:
                    with self.lock:
                        self.history_inflight = None
                        self.status["history_inflight"] = None
                ...
                self._finish_history_request(tf, False, str(exc))
            return
        if typ == "tick" and data.get("tick"):
            ...
            with self.lock:
                ...
                for tf, seconds in self.TF_SECONDS.items():
                    ...
                    # A first live tick is a genuine observed price and can safely
                    # initialize the current in-progress OHLC candle. This is NOT
                    # fabricated history; it is the OHLCV Builder's live tick path.
                    if frame is None or frame.empty:
                        frame = pd.DataFrame([{"time":start,"open":quote,"high":quote,"low":quote,"close":quote,"volume":0.0}])
    ...
    def _run(self):
        while not self.stop_event.is_set():
            try:
                # PUBLIC is the default for read-only market data. Deriv documents
                # ticks/ticks_history as no-auth market-data endpoints. Authentication
                # is retained only as an optional account-scoped mode.
                if self.market_data_mode == "AUTHENTICATED":
                    ...
                    ws_candidates = [self.request_authenticated_ws_url(self.api_base, self.app_id, self.token, self.account_id)]
                else:
                    # Deriv's documented generic public market-data workflow uses the
                    # public WebSocket below for active_symbols, ticks and ticks_history.
                    # Use it FIRST because V14 requires ticks_history with style="candles"
                    # for the historical OHLCV path. The newer Options public endpoint
                    # is retained only as a fallback for environments where the legacy
                    # generic endpoint is unavailable. No authentication is required.
                    # Use Deriv's documented generic public market-data endpoint.
                    # It supports BOTH ticks and ticks_history for Forex market data.
                    # Do not silently fall back to an endpoint that may support live
                    # quotes but not the historical-candle request required to seed
                    # the analysis pipeline.
                    legacy_url = "wss://ws.binaryws.com/websockets/v3"
                    if self.app_id:
                        legacy_url += f"?app_id={self.app_id}"
                    ws_candidates = [legacy_url]
                ...
                for ws_url in ws_candidates:
                    try:
                        with self.lock:
                            self.status["endpoint"] = ws_url.split("?")[0]
                        ...
                        with self.lock:
                            self.status["error"] = str(endpoint_error)
                            self.status["connected"] = False
                            self.status["authenticated"] = False
                            self.status["streaming"] = False

                if not connected_once and last_error is not None:
                    raise last_error
    ...
    def start(self, wait_seconds=120, required_timeframe=None, required_rows=None):
        self.required_timeframe = str(required_timeframe).upper() if required_timeframe else None
        self.required_rows = int(required_rows or self.outputsize)
        # Only callers that explicitly request a timeframe/row target are allowed
        # to arm historical acquisition. A normal live start is tick-only.
        self.history_acquisition_requested = bool(self.required_timeframe in self.TF_SECONDS)
        if self.thread and self.thread.is_alive():
            snap = self.snapshot()
            if self.required_timeframe:
                rows = int(snap.get("history_rows", {}).get(self.required_timeframe, 0))
                if snap.get("connected") and rows >= self.required_rows:
                    return snap
                # A Streamlit rerun may reuse an already-connected stream that
                # was created with a smaller historical target. Re-arm pagination
                # on that existing socket instead of waiting forever for a new
                # on_open event. The request queue remains strictly one-at-a-time.
                if snap.get("connected") and self.ws is not None:
                    with self.lock:
                        tf = self.required_timeframe
                        if tf in self.TF_SECONDS and tf not in self.history_queue and not self.history_complete.get(tf, False):
                            self.history_queue.insert(0, tf)
                    self._send_next_history_request()
        ...
        self.thread.start()
        deadline = _time.time() + max(10, int(wait_seconds))
        while _time.time() < deadline:
            ...
            if self.required_timeframe:
                ...
                return snap
            err = snap.get("error")
            if err and not snap.get("authenticated") and _time.time() + 1 >= deadline:
        ...
        if self.required_timeframe:
            rows = int(snap.get("history_rows", {}).get(self.required_timeframe, 0))
            raise RuntimeError(
                f"Deriv historical acquisition timed out for {self.symbol}/{self.required_timeframe}: "
                f"received {rows:,}/{self.required_rows:,} candles. Diagnostic: {snap}"
            )
        if snap.get("connected") and not self.required_timeframe:
            return snap
        if snap.get("connected") and self.frames:
            return snap
        raise RuntimeError(f"Deriv market-data stream did not produce the requested live/historical data within the startup window: {snap}")
    ...
    def snapshot(self):
        with self.lock:
            ...
            history_complete = dict(self.history_complete)
            history_pages = dict(self.history_pages)
            history_rows = {k: int(len(v)) for k, v in self.frames.items()}
            history_oldest = {k: (str(v) if v is not None else None) for k, v in self.history_oldest.items()}
            history_inflight = dict(self.history_inflight) if self.history_inflight else None
            history_errors = dict(self.history_errors)
        ...
        s["history_complete"] = history_complete
        s["history_pages"] = history_pages
        s["history_rows"] = history_rows
        s["history_oldest"] = history_oldest
        s["history_inflight"] = history_inflight
        s["history_error_by_timeframe"] = history_errors
        s["history_rate_guard"] = {**s.get("history_rate_guard", {}), **DerivHistoryRateGuard.status(self.history_request_key)}
        s["healthy"] = bool(s.get("connected") and s.get("validated") and (s.get("authenticated") or s.get("connection_mode") == "PUBLIC"))
        s["live_tick_healthy"] = bool(s.get("streaming") and (s.get("tick_age_seconds") is not None) and s.get("tick_age_seconds") <= 15)
        return s
...
def get_deriv_live_market_data(symbol: str, timeframe: str, cfg: Config):
    """Unified Deriv real-Forex market-data path for the live dashboard.

    On first connection it obtains a ONE-TIME verified historical seed of at least
    ``minimum_history_candles`` (1,200 by default) from Deriv ``ticks_history``.
    The same persistent WebSocket then subscribes to live ticks.

    Flow:
      Deriv historical candles -> OHLCV Builder -> Validation -> technical engines
      Deriv live ticks -> Tick Normalizer -> OHLCV Builder -> Validation -> same series

    No synthetic prices are created and Streamlit reruns do not repeatedly request
    history because the stream object records that its bootstrap was attempted.
    """
    if websocket is None:
    ...
    target_rows = max(1200, int(getattr(cfg, "minimum_history_candles", 1200) or 1200))
    stream = DerivRealForexStream.get(
        symbol, cfg.deriv_api_base, app_id, token, configured_account,
        max(int(getattr(cfg, "deriv_stream_outputsize", target_rows) or target_rows), target_rows),
        cfg.deriv_stream_reconnect_seconds, market_data_mode
    )
    stream.outputsize = max(int(stream.outputsize), target_rows)
    stream.history_page_size = min(5000, max(1200, target_rows))
    stream.history_max_pages = max(1, int(math.ceil(target_rows / float(stream.history_page_size))) + 1)
    stream.history_min_interval_seconds = max(5, int(getattr(cfg, "deriv_history_min_interval_seconds", 5) or 5))
    stream.history_rate_limit_cooldown_seconds = max(60, int(getattr(cfg, "deriv_history_rate_limit_cooldown_seconds", 120) or 120))

    status = stream.snapshot()
    if not status.get("connected"):
        status = stream.start(
            wait_seconds=max(20, min(60, int(getattr(cfg, "deriv_live_start_timeout_seconds", 45) or 45))),
            required_timeframe=None, required_rows=None
        )

    # Seed the selected timeframe once. If the provider has already rejected a
    # historical request, fail closed without sending another request during a
    # Streamlit rerun. Existing verified frames remain usable.
    frame = stream.frame(timeframe)
    if (frame is None or frame.empty) and not stream.live_history_bootstrap_attempted:
        guard = DerivHistoryRateGuard.status(stream.history_request_key)
        if guard.get("cooldown"):
            raise RuntimeError(
                f"DERIV_RATE_LIMIT_COOLDOWN: historical seed is temporarily blocked for "
                f"approximately {int(guard.get('cooldown_seconds', 0))}s; no duplicate request sent."
            )
        stream.live_history_bootstrap_attempted = True
        stream.live_history_bootstrap_at = datetime.now(timezone.utc).isoformat()
        try:
            status = stream.start(
                wait_seconds=max(30, min(90, int(getattr(cfg, "deriv_history_timeout_seconds", 180) or 180))),
                required_timeframe=timeframe,
                required_rows=target_rows,
            )
            frame = stream.frame(timeframe)
        except Exception:
            # Do not retry automatically on every dashboard rerun. The exact
            # provider diagnostic is returned to the operator.
            raise

    ticks = stream.ticks()
    tick_bars = OHLCVBuilderEngine.build_from_ticks(
        ticks, timeframe, symbol=symbol, source="DERIV REAL FOREX"
    )
    if not tick_bars.empty:
        if frame is None or frame.empty:
            frame = tick_bars
        else:
            frame = OHLCVBuilderEngine._merge_provider_and_tick_bars(frame, tick_bars)
    ...
    if frame is None or frame.empty:
        meta = {
            ...
            "connection_mode": market_data_mode, "live_only": False,
            "historical_request_sent": bool(stream.live_history_bootstrap_attempted),
            "historical_bootstrap_attempted": bool(stream.live_history_bootstrap_attempted),
            "ticks_observed": int(len(ticks)), "stream_health": status,
            "live_tick_healthy": bool(status.get("live_tick_healthy")),
            "ohlcv_builder_status": "WAITING_FOR_VERIFIED_HISTORICAL_OR_LIVE_CANDLE",
            "fetched_at": datetime.now(timezone.utc).isoformat(),
        }
        return _empty_market_data(), meta

    frame = MarketDataEngine.normalize(frame)
    validation = MarketDataEngine.validate(frame)
    if not validation.get("data_ok"):
        raise RuntimeError(f"Deriv {symbol}/{timeframe} live+historical OHLCV validation failed: {validation}")
    if status.get("streaming") and status.get("tick_age_seconds") is not None and status["tick_age_seconds"] > cfg.deriv_stream_stale_seconds:
        raise RuntimeError(f"Deriv real-Forex live tick stream is stale for {symbol}: {status}")
    ...
    meta = {
        ...
        "historical_request_sent": bool(stream.live_history_bootstrap_attempted),
        "historical_bootstrap_attempted": bool(stream.live_history_bootstrap_attempted),
        "historical_rows": int(len(stream.frame(timeframe))) if stream.frame(timeframe) is not None else 0,
        "ticks_observed": int(len(ticks)), "stream_health": stream.snapshot(),
        "fetched_at": datetime.now(timezone.utc).isoformat(),
    }
    return frame, meta


def get_deriv_real_forex_data(symbol: str, timeframe: str, cfg: Config, force=False, required_rows: Optional[int] = None):
    """EXPLICIT HISTORICAL acquisition path for V14 testing/training.

    This is the only live-code path permitted to call Deriv ``ticks_history``.
    It must be invoked by an explicit historical dataset/test operation, never by
    dashboard refresh, MTF analysis, scanner warming, or live connection startup.
    """
    if websocket is None:
        raise RuntimeError("Deriv streaming requires websocket-client. Add websocket-client to requirements.txt.")
    symbol = canonical_symbol(symbol)
    ...
    # Public market data needs no PAT, App ID, account discovery, or OTP.
    # Credentials remain supported for optional authenticated mode.
    market_data_mode = str(getattr(cfg, "deriv_market_data_mode", "PUBLIC") or "PUBLIC").upper()
    if market_data_mode == "AUTHENTICATED":
        app_id, token, configured_account = DerivRealForexStream.resolve_credentials(
            getattr(cfg, "deriv_options_account_id", "")
    ...
    configured_target = max(500, int(getattr(cfg, "historical_target_rows", cfg.deriv_stream_outputsize) or cfg.deriv_stream_outputsize))
    # Dashboard/live bootstrap must be fast: only require the minimum validated
    # working history. The background stream continues acquiring the larger
    # historical target. The dedicated historical-test path passes required_rows
    # explicitly when it needs the full target.
    target_rows = max(1200, int(required_rows if required_rows is not None else getattr(cfg, "minimum_history_candles", 1200) or 1200))
    target_rows = min(target_rows, configured_target)
    stream.outputsize = max(int(stream.outputsize), configured_target)
    # Use the smallest page that can satisfy the current request. Live dashboard
    # bootstrap therefore costs one 1,000-candle request, while the explicit
    # historical test can use 5,000-candle pages for its 12,000-candle target.
    stream.history_page_size = min(5000, max(1200, target_rows))
    stream.history_min_interval_seconds = max(5, int(getattr(cfg, "deriv_history_min_interval_seconds", 5) or 5))
    stream.history_rate_limit_cooldown_seconds = max(60, int(getattr(cfg, "deriv_history_rate_limit_cooldown_seconds", 120) or 120))
    stream.history_max_pages = max(1, int(math.ceil(stream.outputsize / float(stream.history_page_size))) + 1)

    # Fail closed before touching the socket when Deriv has already rejected a
    # historical request. This prevents a Streamlit rerun from turning one 429
    # into a retry storm. Existing validated stream data may still be consumed.
    guard = DerivHistoryRateGuard.status(stream.history_request_key)
    if guard.get("cooldown"):
        existing = stream.frame(timeframe)
        if existing is not None and len(existing) >= target_rows:
            df = existing
            status = stream.snapshot()
        else:
            raise RuntimeError(
                f"DERIV_RATE_LIMIT_COOLDOWN: historical request blocked for approximately "
                f"{int(guard.get('cooldown_seconds', 0))}s. No retry was sent. "
                f"Wait for the provider cooldown and reuse the existing stream."
            )
    else:
        status = stream.start(
            wait_seconds=max(30, min(60, int(getattr(cfg, "deriv_live_start_timeout_seconds", 45) or 45))),
            required_timeframe=timeframe,
            required_rows=target_rows,
        )
        df = stream.frame(timeframe)
    if df is None or df.empty:
        raise RuntimeError(f"Deriv market-data stream did not produce {symbol}/{timeframe} candles. Diagnostic: {status}")

    # The Deriv historical bootstrap is retained, while the live tail is rebuilt
    # from the same Deriv ticks through the V14 OHLCV Builder.
    ...
    # Historical candles are usable as soon as the socket is connected and the
    # returned market data passes validation. The first tick may arrive a moment
    # later, so absence of that first tick must not prevent the dashboard from
    # loading the real Deriv candles. Once ticks exist, enforce the stale limit.
    if not status.get("connected") or not status.get("validated"):
        raise RuntimeError(f"Deriv real-Forex market-data connection is unavailable for {symbol}: {status}")
    if status.get("streaming") and status.get("tick_age_seconds") is not None and status["tick_age_seconds"] > cfg.deriv_stream_stale_seconds:
        raise RuntimeError(f"Deriv real-Forex live tick stream is stale for {symbol}: {status}")
    ...
    meta = {
        ...
        "live_stream_ready": bool(status.get("streaming")),
        "authenticated": bool(market_data_mode == "AUTHENTICATED"),
        "connection_mode": market_data_mode,
        "historical_rows": int(len(df)),
        "historical_target_rows": int(configured_target),
        "required_rows_for_this_call": int(target_rows),
        "historical_complete": bool(len(df) >= target_rows),
        "history_rows_received": int(status.get("history_rows", {}).get(timeframe, len(df))),
        "history_pages_received": int(status.get("history_pages", {}).get(timeframe, 0)),
        "account_type":("demo" if market_data_mode == "AUTHENTICATED" else "public"),
...
def _v14_render_architecture_testing(df, cfg, symbol, timeframe):
    ...
    st.caption("If the selected real provider is below the 1,200-candle statistical minimum, V14 acquires additional provider history through the same provider path. Deriv history is paginated without relying on optional echo_req; Twelve Data uses bounded backward pagination. No synthetic candles are created.")
    st.caption(
...
def dashboard():
    ...
    with st.sidebar:
        ...
        # V14's primary live-market path defaults to Deriv real-Forex public
        # market data. Twelve Data remains available as an explicit provider.
        ...
        if data_source == "TWELVE DATA":
            ...
            cfg.deriv_stream_outputsize = st.number_input(
                "Deriv historical candles", 1200, 20000, 1200, 100
            )
            cfg.deriv_stream_stale_seconds = st.number_input(
                "Live tick stale limit (seconds)", 5, 120, 15, 1
            )
            st.caption(
                "Deriv real-Forex LIVE stream · read-only · ticks → Tick Normalizer → OHLCV Builder. "
                "A verified 1,200-candle Deriv historical seed is acquired once on first live connection, then live ticks update the same OHLCV series. "
                "No synthetic/fabricated market data is used."
            )
            if st.button("🟢 Connect Deriv Real-Forex Data", use_container_width=True):
                try:
                    # Connect only to the persistent live tick stream. Historical
                    # acquisition is intentionally not triggered by this button.
                    live_df, meta = get_deriv_live_market_data(symbol, timeframe, cfg)
                    live_df, meta = store_market_data(
    ...
    if sync_ok:
        ...
        live_error = str((st.session_state.get("live_status", {}) or {}).get("error", "") or "")
        suffix = f" · error={live_error}" if live_error else ""
        st.error(
            f"⛔ DATA SYNC BLOCKED · selected {display_symbol(symbol)}/{timeframe} · "
            f"loaded {display_symbol(loaded_symbol or 'NONE')}/{loaded_tf or 'NONE'}{suffix}"
        )
    ...
    with panels[12]:
        ...
        if central.get("assessment_readiness"):
            st.success(
                f"CENTRAL ASSESSMENT READY · {central.get('rows', len(df)):,} validated rows · "
                f"source={central.get('data_source', resolved_source)}"
            )
        else:
            reasons = central.get("blocking_reasons", []) or ["CENTRAL_ASSESSMENT_BLOCKED"]
            st.warning("CENTRAL ASSESSMENT BLOCKED · " + " · ".join(map(str, reasons)))
    ...
    with panels[15]:
        ...
        st.json({
            ...
            "historical_path": {
                ...
                "minimum_target_rows": int(getattr(cfg, "historical_target_rows", 1200)),
                "test_ready": bool((st.session_state.get("historical_builder_status", {}) or {}).get("status") == "READY"),
            },
            "live_path": {
                "provider": resolved_source,
                "tick_stream": bool((st.session_state.get("live_status", {}) or {}).get("streaming", False)),
                "ohlcv_builder": "TICK_NORMALIZER -> OHLCV_BUILDER",
                "historical_request_from_live_path": False if resolved_source == "DERIV REAL FOREX" else None,
            },
            "central_assessment_input": {
                "rows": central.get("rows", 0),
                "validation_status": central.get("validation_status", validation.get("status", "UNKNOWN")),
                "quality_gate": central.get("quality_gate_status", "BLOCK"),
            },
        })
    ...
    if auto_refresh:
        ...
        try:
            ...
            if callable(auto_refresh_fn):
                auto_refresh_fn(interval=int(refresh_seconds * 1000), key="v14_auto_refresh")
            else:
                st.caption("Automatic refresh is unavailable in this Streamlit version; use the browser refresh control.")
        except Exception as refresh_error:
            st.caption(f"Automatic refresh unavailable: {refresh_error}")
...
if __name__ == "__main__":
    try:
        ...
        # Never hide the real startup/runtime exception behind Streamlit's generic
        # "Error running app" screen. This does not bypass any safety control.
        try:
            st.error("V14 dashboard startup/runtime error")
            st.exception(exc)

...
class D1ValidatedProbabilityEngine:
    """Leakage-aware D1 probability model with a time-ordered validation report.

    This is an additional probability layer, not a replacement for the existing
    AI/ML engine.  It uses only completed D1 observations, validates on a later
    holdout period, and reports accuracy/Brier score before its current-day
    probability is allowed to influence the D1 stack.
    """
    FEATURES = (
        "ret1", "ret3", "ret5", "ema20_gap", "ema50_gap", "rsi",
        "atr_pct", "macd_norm", "adx", "bb_pos"
    )

    @classmethod
    def _features(cls, df):
        x = MarketDataEngine.normalize(df).copy()
        ...
        e20 = ema(x.close, 20)
        e50 = ema(x.close, 50)
        x["ema20_gap"] = x.close / e20.replace(0, np.nan) - 1
        x["ema50_gap"] = x.close / e50.replace(0, np.nan) - 1
        x["rsi"] = rsi(x.close, 14) / 100.0
        x["atr_pct"] = atr(x, 14) / x.close.replace(0, np.nan)
        _, _, mh = macd(x.close)
        atrv = atr(x, 14).replace(0, np.nan)
        x["macd_norm"] = mh / atrv
        x["adx"] = adx(x)[0] / 100.0
        mid = x.close.rolling(20).mean()
        sd = x.close.rolling(20).std().replace(0, np.nan)
        x["bb_pos"] = (x.close - (mid - 2 * sd)) / (4 * sd)
        x = x.replace([np.inf, -np.inf], np.nan).dropna(subset=list(cls.FEATURES))
        return x

    @classmethod
    def predict(cls, df, min_rows=180):
        x = cls._features(df)
        if len(x) < min_rows:
            return {
                "direction": "NEUTRAL", "up_probability": 50.0,
                "down_probability": 50.0, "confidence": 0.0,
                "status": "INSUFFICIENT D1 HISTORY", "validated": False,
                "training_rows": max(0, len(x) - 1), "validation_rows": 0,
            }

        y = (x.close.shift(-1) > x.close).astype(int)
        valid = x.index < y.index[-1]
        x_model = x.loc[valid].copy()
        y_model = y.loc[valid].astype(int)
        if y_model.nunique() < 2 or len(x_model) < min_rows - 1:
            return {
                "direction": "NEUTRAL", "up_probability": 50.0,
                "down_probability": 50.0, "confidence": 0.0,
                "status": "INSUFFICIENT OUTCOME VARIATION", "validated": False,
                "training_rows": len(x_model), "validation_rows": 0,
            }

        split = int(len(x_model) * 0.70)
        split = max(120, min(split, len(x_model) - 30))
        train_x, test_x = x_model.iloc[:split], x_model.iloc[split:]
        train_y, test_y = y_model.iloc[:split], y_model.iloc[split:]
        try:
            ...
            model.fit(train_x[list(cls.FEATURES)], train_y)
            test_prob = model.predict_proba(test_x[list(cls.FEATURES)])[:, 1]
            test_pred = (test_prob >= 0.5).astype(int)
            validation_accuracy = float(accuracy_score(test_y, test_pred) * 100)
            brier = float(brier_score_loss(test_y, test_prob))
            logloss = float(log_loss(test_y, np.clip(test_prob, 1e-6, 1 - 1e-6)))

            # Fit only on observations available before the current D1 observation.
            current_model = Pipeline([
                ("scale", StandardScaler()),
                ("model", LogisticRegression(max_iter=1000, class_weight="balanced")),
            ])
            current_model.fit(x_model[list(cls.FEATURES)], y_model)
            latest = x.iloc[[-1]][list(cls.FEATURES)]
            prob = float(current_model.predict_proba(latest)[0][1])
            model_name = "TIME-ORDERED LOGISTIC D1"
        except Exception as exc:
            return {
                "direction": "NEUTRAL", "up_probability": 50.0,
                "down_probability": 50.0, "confidence": 0.0,
                "status": f"MODEL UNAVAILABLE: {type(exc).__name__}",
                "validated": False, "training_rows": len(train_x),
                "validation_rows": len(test_x),
            }

        # Do not present a weak holdout as a high-confidence probability.
        reliability = float(np.clip(
            100.0 * (1.0 - min(1.0, brier * 2.0)) * 0.65
            + validation_accuracy * 0.35, 0, 100
        ))
        confidence = float(abs(prob - 0.5) * 200 * (reliability / 100.0))
        direction = "BULLISH" if prob >= 0.55 else "BEARISH" if prob <= 0.45 else "NEUTRAL"
        return {
            "direction": direction,
            "up_probability": round(prob * 100, 2),
            "down_probability": round((1 - prob) * 100, 2),
...
class FullStackDailySignalEngine:
    """Full-stack, reliability-aware D1 calculator for completed and forming daily states."""
    EXCLUDED_FROM_DAILY_VOTE = {"daily_signal"}

    @classmethod
    def _ledger(cls, analysis, data_quality, locked_context=None):
        """Build the complete D1 evidence ledger.

        Directional engines influence direction. Every other engine still changes
        the market reading through a normalized context/reliability influence.
        This prevents a neutral risk/data/timing engine from being falsely treated
        as a directional vote while ensuring it cannot be ignored.
        """
        ...
        for label, key, role, weight in EngineInformationBus.SPECS:
            if key in cls.EXCLUDED_FROM_DAILY_VOTE:
                continue
            item = bus.get(label, {}) or {}
            ...
            if directional and ok:
                ...
                vote_weight += float(weight) * scale

            # Every non-directional engine gets a real influence on the final
            # reliability of the D1 reading. It is not converted into a fake BUY/SELL.
            if ok:
                context_score = strength if strength > 0 else 100.0
                if key == "risk":
                    context_score = 100.0 if raw.get("approved", False) else 0.0
                elif key == "risk_veto":
                    context_score = 100.0 if not raw.get("reasons") else 0.0
                elif key == "data_quality":
                    context_score = float(raw.get("score", strength))
                elif key == "signal_timing":
                    context_score = 100.0 if raw.get("fresh", True) else 0.0
                elif key == "session":
                    context_score = 100.0 if raw.get("session_tradeable", False) else 45.0
                elif key == "economic":
                    context_score = 0.0 if raw.get("blocked", False) else float(raw.get("score", strength))
                elif key == "mtf":
                    context_score = float(raw.get("alignment", strength))
                elif key == "d1_validated_probability":
                    context_score = float(raw.get("reliability", strength)) if raw.get("validated") else 35.0
                context_weighted += float(weight) * float(np.clip(context_score, 0, 100))
                context_weight += float(weight)

            rows.append({
                "engine": label, "key": key, "role": role,
                "weight": float(weight), "direction": direction,
                "strength": round(strength, 2), "healthy": ok,
                "available": bool(item.get("available")),
                "directional_vote": bool(directional and ok),
                "contribution": round(contribution, 5),
                "context_score": round(float(np.clip(context_score if ok else 0.0, 0, 100)), 2),
                "sentiment_influence": round(
                    contribution if directional and ok
                    else (float(weight) * (float(np.clip(context_score if ok else 0.0, 0, 100)) / 100.0)), 5
                ),
        ...
        context_quality = context_weighted / max(context_weight, 1e-9) if context_weight else 0.0

        # Require both directional evidence and full-stack health. This is not a
        # promise of accuracy; it is a guard against overconfident partial stacks.
        full_stack_confidence = float(np.clip(
            agreement * 0.45 + participation * 0.20 + context_quality * 0.25
            + min(100.0, abs(normalized) * 100.0) * 0.10,
            0, 100,
        ))
        return {
            "signal": direction,
            "weighted_score": round(weighted, 5),
            "normalized_score": round(normalized, 5),
            "directional_agreement": round(agreement, 1),
            "engine_participation": round(participation, 1),
            "context_quality": round(context_quality, 1),
            "full_stack_confidence": round(full_stack_confidence, 1),
    ...
    def evaluate_completed(cls, df, symbol, cfg):
        ...
        return {"status":"LOCKED" if locked else "UNCONFIRMED","signal":ledger["signal"],"locked":locked,
                ...
                "confidence":round(min(conf, ledger.get("full_stack_confidence", conf)),1),"engine_agreement":ledger["directional_agreement"],"engine_participation":ledger["engine_participation"],"context_quality":ledger.get("context_quality",0.0),"full_stack_confidence":ledger.get("full_stack_confidence",0.0),
...
class CurrentDailyMarketSentimentEngine:
    ...
    def evaluate(cls, forming_report: Dict[str, Any], locked_report: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        confidence = min(100.0, max(0.0, (abs(normalized) * 100.0 * 0.35) + agreement * 0.15 + participation * 0.15 + quality_score * 0.15 + float(forming_report.get("full_stack_confidence", 0.0) or 0.0) * 0.20))
        ...
        return {
            ...
            "context_quality": float(forming_report.get("context_quality", 0.0) or 0.0),
            "full_stack_confidence": float(forming_report.get("full_stack_confidence", 0.0) or 0.0),
...
class EngineInformationBus:
    ...
    SPECS = [
        ...
        ("Validated D1 Probability", "d1_validated_probability", "directional", 1.20),
...
def analyze_market(df, symbol, cfg, timeframe=None, *, deep_mtf=True):
    ...
    d1_validated_probability = (
        D1ValidatedProbabilityEngine.predict(df)
        if str(timeframe or "").upper() == "D1"
        else {"direction": "NEUTRAL", "up_probability": 50.0, "down_probability": 50.0, "confidence": 0.0, "status": "D1 ONLY", "validated": False}
    )
    ...
    advisory["ai"] = ai
    advisory["d1_validated_probability"] = d1_validated_probability
    ...
    result = {
        ...
        "ai": ai,
        "d1_validated_probability": d1_validated_probability,
        "ensemble": ensemble,
        "no_trade": no_trade,
    ...
    return _attach_daily_context(result, daily_context)
...
def build_full_pair_signal_snapshot(sym, x, cfg, timeframe):
    ...
    # A trade-grade scanner signal is only emitted when the complete V13
    # eligibility stack passes. Otherwise the row remains WAIT with reasons.
    signal = final_direction if live_gate.get("eligible") else "WAIT"
